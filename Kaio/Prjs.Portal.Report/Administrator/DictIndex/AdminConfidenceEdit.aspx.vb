Public Class AdminConfidenceEdit
    Inherits GlobalPage
#Region "Page Load"
        ...
        If Not Me.IsPostBack Then
            ...
            If IsNumeric(ViewState(ViewStateInfo.Object_Id)) Then
                If ViewState(ViewStateInfo.Object_Id) > 0 Then
                    Me.lbltitle.Text = "SỬA ĐỔI MỨC TÍN NHIÊM CỦA ĐỐI TÁC"
                    bindData(ViewState(ViewStateInfo.Object_Id))
                    ViewState(ViewStateInfo.Status) = Constants.Action.Update
                Else
                    Me.lbltitle.Text = "THÊM MỨC TÍN NHIÊM CỦA ĐỐI TÁC"
...
#Region "Bind Data"
    Private Sub bindData(ByVal intId As Integer)
        Dim sql As String = "SELECT * FROM System_Appraise_Confidence Where Id=" & intId
        Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
        If dt.Rows.Count > 0 Then
            Me.txtConfidence_Text.Text = dt.Rows(0).Item("Confidence_Text")
            Me.txtConfidence_Code.Text = IIf(IsDBNull(dt.Rows(0).Item("Confidence_Code")) = True, "", dt.Rows(0).Item("Confidence_Code"))
...
#Region "Submit Click"
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnUpdate.Click
        If Me.txtConfidence_Text.Text = "" Then
            Me.lblerror.Text = "Tên dịch vụ không hợp lệ !"
            Exit Sub
        End If
        If Me.txtConfidence_Text.Text.Trim = "" Then
            Me.lblerror.Text = "Mã dịch vụ không hợp lệ !"
            Exit Sub
        End If
        Dim Confidence_Text As String = Me.txtConfidence_Text.Text.Trim
        Dim Confidence_Code As String = Me.txtConfidence_Code.Text.Trim
        Dim Description As String = Me.txtDescription.Text.Trim
        Dim sql As String = "SELECT * From  System_Appraise_Confidence  Where   Id <>" & ViewState(ViewStateInfo.Object_Id) & " And Confidence_Text=N'" & Confidence_Text & "'"
        If Me.IsCheckData(sql) = False Then
            If ViewState(ViewStateInfo.Status) = Constants.Action.Insert Then
                sql = "Insert Into System_Appraise_Confidence(Confidence_Text, Confidence_Code, Description)" & _
                    "Values (N'" & Confidence_Text & "',N'" & Confidence_Code & "',N'" & Description & "')"
            ElseIf ViewState(ViewStateInfo.Status) = Constants.Action.Update Then
                sql = "Update System_Appraise_Confidence Set Confidence_Text=N'" & Confidence_Text & "'," & _
                "Confidence_Code=N'" & Confidence_Code & "'," & _
                "Description=N'" & Description & "'" & _
                " Where Id=" & ViewState(ViewStateInfo.Object_Id)
            End If
            Try
                MSSQLEnv.ExecuteNonQuery.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
                RedirectUrl(Constants.Url._Admin.AdminConfidenceList)
    ...
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDelete.Click
        Dim str As String = ""
        Dim sql As String = "Delete From System_Appraise_Confidence Where Id=" & ViewState(ViewStateInfo.Object_Id)
        Try
            MSSQLEnv.ExecuteNonQuery.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
            RedirectUrl(Constants.Url._Admin.AdminConfidenceList)
        Catch ex As Exception
