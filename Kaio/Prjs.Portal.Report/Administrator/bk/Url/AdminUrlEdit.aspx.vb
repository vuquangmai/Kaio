...
#Region "Page Load"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Me.IsPostBack Then
            BindChannel()
            bindTypeOfMenu()
            BindParent(Me.DropDownListChannel_Id.SelectedItem.Value)
...
#Region "DictIndex"
    Private Sub BindChannel()
        ...
        End If
    End Sub
    Private Sub BindParent(Channel_Id As Integer)
        Dim sql As String = "SELECT * FROM System_Url Where Url_Level=2 And Is_Locked=0 And Channel_Id=" & Channel_Id & " Order by Url_Text"
        Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
        ...
        If dt.Rows.Count > 0 Then
            For i As Integer = 0 To dt.Rows.Count - 1
                Me.DropDownListParent_Id.Items.Add(New ListItem(dt.Rows(i).Item("Url_Text"), dt.Rows(i).Item("ID")))
            Next
        End If
    End Sub
#End Region
...
#Region "Submit Click"
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnUpdate.Click
        ...
        If ViewState(ViewStateInfo.Status) = Constants.Action.Insert Then
            sql = "Insert Into System_Url(Url_Text, Url_Id, Parent_Id, Channel_Id, Url_Order, Url_Display, Url_Private, Url_Privilege, Url_Level, Is_Locked, Description)" & _
                "Values (N'" & Url_Text & "',N'" & Url_Id & "',N'" & Parent_Id & "',N'" & Channel_Id & "',N'" & Url_Order & "',N'" & Url_Display & "',N'" & Url_Private & "',N'" & Url_Privilege & "',N'" & Url_Level & "',N'" & Is_Locked & "',N'" & Description & "')"
