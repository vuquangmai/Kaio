...
Public Class BilBrandInitDataList
    Inherits GlobalPage
    Public Utils As New Util.Encrypt
#Region "Page Load"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Me.IsPostBack Then
            Me.lbltitle.Text = "KHỞI TẠO DỮ LIỆU ĐỐI SOÁT THEO HỢP ĐỒNG -  DỊCH VỤ SMS BRAND NAME"
            BindYear()
            BindMonth()
...
#Region "Bind Data"
    Private Sub BindData(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        Dim sql As String = "SELECT A.Id, A.Year, A.Month, A.Contract_Id, A.Contract_Code, A.Partner_Id,A.Partner_Text,A.Partner_Code,A.Account_Report, " & _
             ...
             " From Billing_Brand_Work_Follow  A  " & _
             " Where Year=" & Me.DropDownListYear.SelectedItem.Value & " And Month=" & Me.DropDownListMonth.SelectedItem.Value
        If Me.txtAccount_Report.Text.Trim <> "" Then
            sql = sql & " AND A.Account_Report like N'%" & Me.txtAccount_Report.Text.Trim & "%'"
        End If
        If Me.txtPartner_Text.Text.Trim <> "" Then
            sql = sql & " AND A.Partner_Text like N'%" & Me.txtPartner_Text.Text.Trim & "%'"
        End If
...
#Region "Submit Click"
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnUpdate.Click
        ...
        Dim Dept_Id_Biz As Integer = 0
        Dim Dept_Text_Biz As String = 0
        ...
        If Me.CheckBoxDel.Checked = True Then
            sql = "Delete  From  Billing_Brand_Work_Follow  Where   Year =" & Year & " And Month=" & Month
            Try
            ...
            Catch ex As Exception
                ...
                Exit Sub
            End Try
            sql = "Delete  From  Billing_Brand_Work_Follow_Detail  Where   Year =" & Year & " And Month=" & Month
            Try
                MSSQLEnv.ExecuteNonQuery.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
            Catch ex As Exception
                Me.lblerror.Text = "Lỗi xóa dữ liệu cũ. Mã lỗi: " & ex.Message
            ...
            End Try
        End If
        sql = "SELECT  SUM(Hour_Implement) TotalHour_Implement,SUM(Day_Implement) TotalDay_Implement, COUNT(*) Total_Task FROM   Billing_Brand_DictIndex_Task "
        ...
        sql = "Select * From Billing_Brand_DictIndex_Task Where Task_Order=1"
        ...
        Else
            ...
            Exit Sub
        End If
        sql = "Select * From Billing_Brand_DictIndex_Task Where Task_Order=2"
        ...
        sql = "SELECT A.Id,A.Contract_Code,A.Contract_Number,A.Cycle_Contract_Text, A.Contract_Text,Isnull(A.Account_Report,'') Account_Report," & _
            " B.Service_Text ,B.Service_Code,C.Model_Text,C.Model_Code,D.Id Partner_Id,D.Partner_Text,D.Partner_Code " & _
            ...
            " WHERE   A.Service_Id=" & Constants.ServiceInfo.Id.BN & _
...
#Region "Init Data"
    Private Function InitData(ByVal Year As Integer, _
        ...
        With cmd
            ...
            .CommandText = "Billing_Brand_Work_Follow_Init_Data"
...
#Region "Delete Data"
    Sub DelData(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs)
        'Dim sql As String = "Delete  From  Billing_Brand_DictIndex_Task  Where  Id =" & CType(e.CommandArgument, Integer)
        'Try
        '    MSSQLEnv.ExecuteNonQuery.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
        '    ViewState("DATA_GRID") = Nothing
        '    ViewState("DATA_COUNT") = Nothing
...
#Region "Ajax"
    Protected Sub DropDownListMonth_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DropDownListMonth.SelectedIndexChanged
        ...
        BindData(intPageSize, intCurentPage, Constants.Action.Search)
    End Sub
    Protected Sub txtPartner_Text_TextChanged(sender As Object, e As EventArgs) Handles txtPartner_Text.TextChanged
        ViewState("DATA_GRID") = Nothing
        ViewState("DATA_COUNT") = Nothing
        Dim intPageSize As Integer = pager1.PageSize
        Dim intCurentPage As Integer = pager1.CurrentIndex
        BindData(intPageSize, intCurentPage, Constants.Action.Search)
    End Sub
    Protected Sub txtAccount_Report_TextChanged(sender As Object, e As EventArgs) Handles txtAccount_Report.TextChanged
        ViewState("DATA_GRID") = Nothing
        ViewState("DATA_COUNT") = Nothing
        Dim intPageSize As Integer = pager1.PageSize
        Dim intCurentPage As Integer = pager1.CurrentIndex
