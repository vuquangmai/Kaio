...
Public Class BilBrandWorkFollowEdit
    Inherits GlobalPage
#Region "Page Load"
        ...
        If Not Me.IsPostBack Then
            ...
            If IsNumeric(ViewState(ViewStateInfo.Object_Id)) Then
                If ViewState(ViewStateInfo.Object_Id) > 0 Then
                    Me.lbltitle.Text = "CẬP NHẬT THÔNG TIN TIẾN ĐỘ ĐỐI SOÁT VỚI ĐỐI TÁC - DỊCH VỤ SMS BRAND"
...
#Region "DictIndex"
    Private Sub BindTask(ByVal Task_Id_Current As Integer)
        Dim sql As String = "SELECT * FROM Billing_Brand_DictIndex_Task Where Id<> " & Task_Id_Current & " Order by Task_Order"
...
#Region "Bind Next Task Auto"
    Private Sub BindNextTaskAuto()
        Dim sql As String = "SELECT * FROM Billing_Brand_DictIndex_Task Where Task_Order=" & CInt(Me.lblTask_Order_Current.Text.Trim) + 1
        Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
        If dt.Rows.Count > 0 Then
...
#Region "Submit Click"
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnUpdate.Click
        ...
        If Status_Id_Current = 1 Then ' Hoàn thành bước hiện tại và chuyển bước tiếp theo
            ...
            Dim sql As String = "SELECT * FROM Billing_Brand_Work_Follow Where Id=" & ViewState(ViewStateInfo.Object_Id)
            Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
            If dt.Rows.Count > 0 Then
                ...
                Dim Dept_Id_Biz As Integer = dt.Rows(0).Item("Dept_Id_Biz")
                ...
                Dim Dept_Id_Execute_Current As Integer = dt.Rows(0).Item("Dept_Id_Execute_Current")
                Dim Dept_Text_Execute_Current As String = dt.Rows(0).Item("Dept_Text_Execute_Current")
                Dim Dept_Id_Execute_Next As Integer = dt.Rows(0).Item("Dept_Id_Execute_Next")
                ...
                Dim Total_Payment As Decimal = dt.Rows(0).Item("Total_Payment")
        ...
        If retval = "" Then
            RedirectUrl(Constants.Url._Billing.BilBrandWorkFollowList)
        Else
...
#End Region
#Region "Update Data"
    Private Function UpdatNext_Old() As String
        Dim retval As String = ""
        Dim sql As String = ""
        ...
        sql = "Select * From Billing_Brand_DictIndex_Task Where Task_Order=" & CInt(Me.lblTask_Order_Current.Text.Trim) + 1
        ...
        If dtTaskCurrent.Rows.Count > 0 Then ' Còn bước tiếp theo
            ...
            sql = "Select * From Billing_Brand_DictIndex_Task Where Task_Order=" & CInt(Me.lblTask_Order_Current.Text.Trim) + 2
            ...
            End If
            sql = "Update Billing_Brand_Work_Follow SET " & _
            ...
            Catch ex As Exception
                ...
                Return retval
            End Try
        Else ' Không còn bước tiếp theo (kết thúc quá trình)
            Status_Id = 1
    ...
    Private Function UpdatNext() As String
        ...
        Dim Status_Id As Integer = 0
        Dim Status_Text As String = ""
        Dim Status_Debts_Id As Integer = StatusDebtsId(Total_Payment, Total_Debts)
        Dim Status_Debts_Text As String = StatusDebtsText(Total_Payment, Total_Debts)
        Dim Update_Time As String = DateTime.Parse(Now).ToString("yyyy-MM-dd HH:mm:ss")
        Dim Update_By_Id As String = CurrentUser.UserId
        Dim Update_By_Text As String = CurrentUser.UserName
        Dim Description As String = Me.txtComment_Current.Text.Trim
        ...
        If Me.DropDownListNextTask.SelectedItem.Value = 1 Then
            sql = "Select * From Billing_Brand_DictIndex_Task Where Task_Order=" & CInt(Me.lblTask_Order_Next_Auto.Text.Trim)
        Else
            sql = "Select * From Billing_Brand_DictIndex_Task Where Task_Order=" & CInt(Me.lblTask_Order_Next_Manual.Text.Trim)
        End If
        Dim dtTaskCurrent As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
        If dtTaskCurrent.Rows.Count > 0 Then
            ...
            If Me.DropDownListNextTask.SelectedItem.Value = 1 Then
                sql = "Select * From Billing_Brand_DictIndex_Task Where Task_Order=" & CInt(Me.lblTask_Order_Next_Auto.Text.Trim) + 1
            Else
                sql = "Select * From Billing_Brand_DictIndex_Task Where Task_Order=" & CInt(Me.lblTask_Order_Next_Manual.Text.Trim) + 1
            End If
            Dim dtTaskNext As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
            If dtTaskNext.Rows.Count > 0 Then
                Task_Order_Next = dtTaskNext.Rows(0).Item("Task_Order")
...
#Region "Insert Data"
    Private Function InsertData(ByVal Work_Follow_Id As Integer, _
                    ...
                    ByVal Dept_Id_Biz As Integer, _
                    ...
                    ByVal Dept_Id_Execute_Current As Integer, _
                    ByVal Dept_Text_Execute_Current As String, _
                    ByVal Dept_Id_Execute_Next As Integer, _
        ...
        With cmd
            ...
            .CommandText = "Billing_Brand_Work_Follow_Detail_Insert"
            ...
            .Parameters.Add(New SqlParameter("@Dept_Id_Biz", SqlDbType.Int))
            ...
            .Parameters.Add(New SqlParameter("@Dept_Id_Execute_Current", SqlDbType.Int))
            ...
            .Parameters.Add(New SqlParameter("@Total_Payment", SqlDbType.Decimal))
            .Parameters("@Total_Payment").Value = Total_Payment
...
#Region "Ajax"
    ...
    Protected Sub DropDownListTask_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DropDownListTask.SelectedIndexChanged
        Dim sql As String = "SELECT * FROM Billing_Brand_DictIndex_Task Where Task_Order=" & CInt(Me.DropDownListTask.SelectedItem.Value)
        Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
        If dt.Rows.Count > 0 Then
