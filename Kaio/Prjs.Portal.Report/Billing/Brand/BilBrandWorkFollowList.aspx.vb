...
Public Class BilBrandWorkFollowList
    Inherits GlobalPage
...
#Region "Page Load"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Me.IsPostBack Then
            Me.lbltitle.Text = "QUẢN LÝ THÔNG TIN TIẾN ĐỘ ĐỐI SOÁT, THANH TOÁN VỚI ĐỐI TÁC - DỊCH VỤ SMS BRAND"
            BindYear()
            BindMonth()
            BindPartner()
            DeptExe()
...
#Region "DictIndex"
    ...
    Private Sub BindPartner()
        ...
        " WHERE B.Service_Id= " & Constants.ServiceInfo.Id.BN
    ...
    Private Sub BindTask()
        Dim sql As String = "SELECT * FROM Billing_Brand_DictIndex_Task  Order by Task_Order "
        Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
    ...
    End Sub
    Private Sub DeptExe()
        Dim sql As String = "SELECT Dept_Id_Execute,Dept_Text_Execute FROM Billing_Brand_DictIndex_Task GROUP BY Dept_Id_Execute,Dept_Text_Execute ORDER BY Dept_Text_Execute"
        Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
...
#Region "RadGrid Event"
    Private Sub RadGrid_NeedDataSource(ByVal source As Object, ByVal e As GridNeedDataSourceEventArgs) Handles RadGrid.NeedDataSource
        If Not e.IsFromDetailTable Then
            Dim sql As String = "SELECT * FROM Billing_Brand_Work_Follow " & _
            ...
            End If
            If Me.txtTotal_Revenue.Text.Trim <> "" Then
                If IsNumeric(Me.txtTotal_Revenue.Text.Trim) Then
                    sql = sql & " And Total_Revenue " & Me.DropDownListHave_Revenue.SelectedItem.Value & Me.txtTotal_Revenue.Text.Trim
                End If
            End If
...
#End Region
#Region "InVisible"
    Public Function SetVisiblIcon(ByVal Dept_Id As Integer) As String
        Dim str As String = "False"
        If Dept_Id = CurrentUser.DeptId Then
            str = "True"
        Else
            str = "False"
        End If
...
#Region "Submit Click"
    Protected Sub btnSearching_Click(sender As Object, e As EventArgs) Handles btnSearching.Click
        ...
        If Me.txtTotal_Revenue.Text.Trim <> "" Then
            If IsNumeric(Me.txtTotal_Revenue.Text.Trim) Then
                sql = sql & " And Total_Revenue " & Me.DropDownListTotal_Revenue.SelectedItem.Value & Me.txtTotal_Revenue.Text.Trim
            End If
        End If
        sql = sql & " ORDER BY Partner_Code"
        Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
    ...
    Protected Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnExp.Click
        ...
        End If
        sql = sql & " ORDER BY Partner_Code"
        ExportData.ExportExcel._Bil.BilScratchWorkFollowList(sql, CurrentUser.UserFullName)
