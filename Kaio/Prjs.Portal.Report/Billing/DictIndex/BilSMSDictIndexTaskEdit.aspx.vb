...
Public Class BilSMSDictIndexTaskEdit
    Inherits GlobalPage
#Region "Page Load"
        ...
        If Not Me.IsPostBack Then
            ...
            If IsNumeric(ViewState(ViewStateInfo.Object_Id)) Then
                If ViewState(ViewStateInfo.Object_Id) > 0 Then
                    Me.lbltitle.Text = "SỬA ĐỔI BƯỚC ĐỐI SOÁT DỊCH VỤ SMS"
                    bindData(ViewState(ViewStateInfo.Object_Id))
                    ViewState(ViewStateInfo.Status) = Constants.Action.Update
                Else
                    Me.lbltitle.Text = "THÊM BƯỚC ĐỐI SOÁT DỊCH VỤ SMS"
...
#Region "Bind Data"
    Private Sub bindData(ByVal intId As Integer)
        Dim sql As String = "SELECT * FROM Billing_SMS_DictIndex_Task Where Id=" & intId
        Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
        If dt.Rows.Count > 0 Then
...
#Region "Submit Click"
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnUpdate.Click
        ...
        Dim sql As String = ""
        sql = "SELECT * From Billing_SMS_DictIndex_Task  Where   Id <>" & ViewState(ViewStateInfo.Object_Id) & " And Task_Name=N'" & Task_Name & "'"
        If Me.IsCheckData(sql) = False Then
            If ViewState(ViewStateInfo.Status) = Constants.Action.Insert Then
                sql = "Insert Into Billing_SMS_DictIndex_Task(Task_Order, Task_Name,Task_End,Hour_Implement, Day_Implement,Dept_Id_Biz,Dept_Text_Biz,Multi_Dept_Biz,Dept_Id_Execute,Dept_Text_Execute,Multi_Dept_Execute,Status, Create_Time, Create_By_Id, Create_By_Text, Update_Time, Update_By_Id,Update_By_Text,Description)" & _
            ...
            ElseIf ViewState(ViewStateInfo.Status) = Constants.Action.Update Then
                sql = "Update Billing_SMS_DictIndex_Task Set Task_Order=N'" & Task_Order & "'," & _
            ...
            Try
                ...
                RedirectUrl(Constants.Url._Billing.BilSMSDictIndexTaskList)
    ...
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDelete.Click
        Dim sql As String = "Delete From Billing_SMS_DictIndex_Task Where Id=" & ViewState(ViewStateInfo.Object_Id)
        Try
            MSSQLEnv.ExecuteNonQuery.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
            RedirectUrl(Constants.Url._Billing.BilSMSDictIndexTaskList)
        Catch ex As Exception
...
#End Region
    ...
    Protected Sub CheckBoxListDept_Id_Execute_SelectedIndexChanged(sender As Object, e As EventArgs) Handles CheckBoxListDept_Id_Execute.SelectedIndexChanged
