...
Public Class BilSMSWorkFollowEdit
    Inherits GlobalPage
#Region "Page Load"
        ...
        If Not Me.IsPostBack Then
            ...
            If IsNumeric(ViewState(ViewStateInfo.Object_Id)) Then
                If ViewState(ViewStateInfo.Object_Id) > 0 Then
                    Me.lbltitle.Text = "CẬP NHẬT THÔNG TIN TIẾN ĐỘ ĐỐI SOÁT VỚI ĐỐI TÁC - DỊCH VỤ SMS MO"
...
#Region "DictIndex"
    Private Sub BindTask(ByVal Task_Id_Current As Integer)
        Dim sql As String = "SELECT * FROM Billing_SMS_DictIndex_Task Where Id<> " & Task_Id_Current & " Order by Task_Order"
...
#Region "Bind Data"
    Private Sub bindData(ByVal intId As Integer)
        Dim sql As String = "SELECT * FROM Billing_SMS_Work_Follow Where Id=" & intId
        Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
        If dt.Rows.Count > 0 Then
...
#Region "Bind Next Task Auto"
    Private Sub BindNextTaskAuto()
        Dim sql As String = "SELECT * FROM Billing_SMS_DictIndex_Task Where Task_Order=" & CInt(Me.lblTask_Order_Current.Text.Trim) + 1
        Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
        If dt.Rows.Count > 0 Then
...
#Region "Submit Click"
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnUpdate.Click
        ...
        If Status_Id_Current = 1 Then ' Hoàn thành bước hiện tại và chuyển bước tiếp theo
            ...
            Dim sql As String = "SELECT * FROM Billing_SMS_Work_Follow Where Id=" & ViewState(ViewStateInfo.Object_Id)
            Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
            If dt.Rows.Count > 0 Then
        ...
        If retval = "" Then
            RedirectUrl(Constants.Url._Billing.BilSMSWorkFollowList)
        Else
...
#Region "Update Data"
    Private Function UpdatNext() As String
        ...
        If Me.DropDownListNextTask.SelectedItem.Value = 1 Then
            sql = "Select * From Billing_SMS_DictIndex_Task Where Task_Order=" & CInt(Me.lblTask_Order_Next_Auto.Text.Trim)
        Else
            sql = "Select * From Billing_SMS_DictIndex_Task Where Task_Order=" & CInt(Me.lblTask_Order_Next_Manual.Text.Trim)
        End If
        ...
        If dtTaskCurrent.Rows.Count > 0 Then
            ...
            If Me.DropDownListNextTask.SelectedItem.Value = 1 Then
                sql = "Select * From Billing_SMS_DictIndex_Task Where Task_Order=" & CInt(Me.lblTask_Order_Next_Auto.Text.Trim) + 1
            Else
                sql = "Select * From Billing_SMS_DictIndex_Task Where Task_Order=" & CInt(Me.lblTask_Order_Next_Manual.Text.Trim) + 1
            End If
            ...
            End If
            sql = "Update Billing_SMS_Work_Follow SET " & _
...
#Region "Insert Data"
    Private Function InsertData(ByVal Work_Follow_Id As Integer, _
        ...
        With cmd
            ...
            .CommandText = "Billing_SMS_Work_Follow_Detail_Insert"
