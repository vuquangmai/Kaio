Public Class CardChargingBizLoger
    Inherits GlobalPage
...
#Region "Bind Data"
    Private Sub BindData(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        sql = "SELECT C.cardName vServiceName  , date_format(A.autotimestamp,'%Y-%m-%d %H:%i:%s') vTime, " & _
            " IF(A.status=0,'success','error') vStatus  , A.userName vUserName , A.serial  vSerial, A.pin vPin, " & _
            " A.error_message vError_Message , A.cardAmount  vCardAmount, B.subcp_name vSubcp_name, " & _
            " B.center_name  vCenter_name  FROM " & _
            vTable & " A LEFT JOIN subcp B  ON A.SubCPId=B.Id LEFT JOIN cardtype C  on A.cardtype=C.cardID"
        ...
        sb = New StringBuilder()
            ...
            Else
                ...
                End If
            End If
        If Me.txtSerial.Text.Trim <> "" Then
            sqlConditional = sqlConditional & " AND Serial='" & Me.txtSerial.Text.Trim & "'"
        End If
        If Me.txtPin.Text.Trim <> "" Then
            sqlConditional = sqlConditional & " AND PIN='" & Me.txtPin.Text.Trim & "'"
        End If
        sqlExc = sql & sqlConditional
        sqlTotal = "SELECT COUNT(*) Total FROM (" & sqlExc & ") A"
