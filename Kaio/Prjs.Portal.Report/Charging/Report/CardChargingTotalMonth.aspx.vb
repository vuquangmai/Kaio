Public Class CardChargingTotalMonth
    Inherits GlobalPage
...
#Region "Page load"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Me.IsPostBack Then
            Me.lbltitle.Text = "BÁO CÁO DOANH THU THẺ CÀO TỔNG HỢP THEO NGÀY"
            BindDictIndex()
            InitData()
...
#Region "Bind Data"
    Private Sub BindData(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        Dim sqlConditional As String = " WHERE substring(time,5,2)='" & Util.StringBuilder.ConvertDigit(Me.DropDownListMonth.SelectedItem.Value) & "'"
        Dim sqlGroup As String = " GROUP BY substring(time,1,4),substring(time,5,2)"
        Dim sqlCriteria As String = ""
        Dim sqlOrder As String = ""
        Dim vTable As String = "tonghopcardlog" & Me.DropDownListYear.SelectedItem.Value

        sql = "SELECT substring(time,1,4) vYear,substring(time,5,2) vMonth," & _
        " if('" & Me.CheckBoxDate.Checked & "' like 'False' ,'--all--',substring(time,7,2)) vDate,  " & _
        " if('" & Me.CheckBoxHour.Checked & "' like 'False' ,'--all--',substring(time,9,2)) vHour,  " & _
        ...
        If Me.CheckBoxAllDate.Checked = False Then
            ...
            sqlConditional = sqlConditional & " And Time>=" & vFromTime
            sqlConditional = sqlConditional & " And Time<=" & vToTime
        End If
        ...
        If Me.CheckBoxDate.Checked Then
            sqlGroup = sqlGroup & ", substring(time,7,2)"
            sqlOrder = sqlOrder & ",CAST(substring(time,7,2)  as SIGNED) "
        End If
        If Me.CheckBoxHour.Checked Then
            sqlGroup = sqlGroup & ", substring(time,9,2)"
            sqlOrder = sqlOrder & ",CAST(substring(time,9,2)  as SIGNED)"
        End If
        ...
        If strAction = Constants.Action.Search Then
            ...
            Dim dt As DataTable = MySQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MySQLConnectionStringCardCharging), sqlExc)
            Dim dtPageCount As DataTable = MySQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MySQLConnectionStringCardCharging), sqlTotal)
        ...
        ElseIf strAction = Constants.Action.Export Then
            ...
            ExportData.ExportExcel._Charging.ChargingTotalMonth(sqlExc, CurrentUser.UserName, _
