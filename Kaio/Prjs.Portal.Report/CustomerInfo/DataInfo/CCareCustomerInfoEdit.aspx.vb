Public Class CCareCustomerInfoEdit
    Inherits GlobalPage
#Region "Page Load"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Me.IsPostBack Then
            ...
            If IsNumeric(ViewState(ViewStateInfo.Object_Id)) Then
                ...
                Else
                    Me.lbltitle.Text = "THÊM THÔNG TIN KHÁCH HÀNG"
...
#Region "Bind Data"
    Private Sub bindData(ByVal intId As Integer)
        Dim sql As String = "SELECT * FROM CCARE_CUSTOMER_INFO Where Id=" & intId
        Dim dt As DataTable = OracleEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(OracleConnectionStringCcare), sql)
        If dt.Rows.Count > 0 Then
            ...
            Me.DropDownListDISTRICT_ID.SelectedValue = dt.Rows(0).Item("DISTRICT_ID")
...
#Region "Submit Click"
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnUpdate.Click
        ...
        Dim LOGER_INFO As String = "- " & DateTime.Parse(Now).ToString("yyyy-MM-dd HH:mm:ss") & ", " & _
                                                                     CurrentUser.UserName & "(" & CurrentUser.UserFullName & ") Nhập thông tin khách hàng <br>"
        Dim sql As String = ""
        Dim retval As String = ""
        sql = "SELECT * From  CCARE_CUSTOMER_INFO  Where   Id <>" & ViewState(ViewStateInfo.Object_Id) & " And upper(USER_ID)='" & USER_ID.ToUpper & "'"
        If Me.B2CCheckData(sql) = False Then
            If ViewState(ViewStateInfo.Status) = Constants.Action.Insert Then
                retval = UpdateObject.B2CManagement.InsertCustomerInfo(USER_ID, _
                                                                                                     STATUS_ID, _
                                                                                                     STATUS_TEXT, _
                                                                                                     GROUP_TEXT, _
                                                                                                     MT, _
            ...
            ElseIf ViewState(ViewStateInfo.Status) = Constants.Action.Update Then
                LOGER_INFO = "- " & DateTime.Parse(Now).ToString("yyyy-MM-dd HH:mm:ss") & ", " & _
                                                                    CurrentUser.UserName & "(" & CurrentUser.UserFullName & ") Sửa thông tin khách hàng <br>"
                retval = UpdateObject.B2CManagement.UpdateCustomerInfo(ViewState(ViewStateInfo.Object_Id), _
                                                                                                         USER_ID, _
                                                                                                         STATUS_ID, _
                                                                                                         STATUS_TEXT, _
                                                                                                         GROUP_TEXT, _
            ...
            End If
            If retval = "" Then
                Me.lblerror.Text = "Cập nhật dữ liệu thành công !"
    ...
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDelete.Click
        Dim sql As String = "Delete  From  CCARE_CUSTOMER_INFO  Where  Id =" & ViewState(ViewStateInfo.Object_Id)
        Try
...
#Region "Ajax"
    ...
    Protected Sub txtUSER_ID_TextChanged(sender As Object, e As EventArgs) Handles txtUSER_ID.TextChanged
        'Dim vPhone As String = Util.FilterNumber(Me.txtUSER_ID.Text.Trim)
        ...
        'If vPhone.StartsWith("849") = True And vPhone.Length <> 11 Then
        '    Me.lblerror.Text = "Định dạng số điện thoại sai (thừa thiếu số)"
        '    Exit Sub
        'End If
        'If vPhone.StartsWith("841") = True And vPhone.Length <> 12 Then
        '    Me.lblerror.Text = "Định dạng số điện thoại sai (thừa thiếu số)"
        '    Exit Sub
        'End If

        'Dim MobileOperator As String = Util.FormatOperator(vPhone)
        'If MobileOperator = "Unknown" Then
        '    Me.lblerror.Text = "Không xác định được thuộc mạng nào !"
        '    Exit Sub
        'End If
        'If ViewState(ViewStateInfo.Status) = Constants.Action.Insert Then
        '    Dim sql As String = "SELECT * From  CCARE_CUSTOMER_INFO  Where   Id <>" & ViewState(ViewStateInfo.Object_Id) & " And upper(USER_ID)='" & vPhone & "'"
        '    If Me.B2CCheckData(sql) = True Then
        '        Me.lblerror.Text = "Số điện thoại này đã tồn tại trên hệ thống !"
        '        Exit Sub
        '    Else
        '        Me.lblerror.Text = "Số điện thoại này chưa có  trên hệ thống !"
...
#End Region
    ...
    Protected Sub ImagebtnCheck_Click(sender As Object, e As ImageClickEventArgs) Handles ImagebtnCheck.Click
        Dim vPhone As String = Util.FilterNumber(Me.txtUSER_ID.Text.Trim)
