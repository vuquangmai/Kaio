Public Class CcareDictIndexDistrictEdit
    Inherits GlobalPage
#Region "Page Load"
...
#Region "Bind Data"
    Private Sub bindData(ByVal intId As Integer)
        Dim sql As String = "SELECT * FROM CCARE_DICTINDEX_DISTRICT Where Id=" & intId
        Dim dt As DataTable = OracleEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(OracleConnectionStringCcare), sql)
        If dt.Rows.Count > 0 Then
            Me.DropDownListPROVINCE_ID.SelectedValue = dt.Rows(0).Item("PROVINCE_ID")
            Me.txtDISTRICT_TEXT.Text = dt.Rows(0).Item("DISTRICT_TEXT")
...
#Region "Submit Click"
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnUpdate.Click
        If Me.DropDownListPROVINCE_ID.SelectedItem.Value = 0 Then
            Me.lblerror.Text = "Tỉnh/Thành không hợp lệ !"
            Exit Sub
        End If
        If Me.txtDISTRICT_TEXT.Text = "" Then
            Me.lblerror.Text = "Quận, Huyện không hợp lệ !"
            Exit Sub
        End If
        Dim PROVINCE_ID As Integer = Me.DropDownListPROVINCE_ID.SelectedItem.Value
        Dim PROVINCE_TEXT As String = Me.DropDownListPROVINCE_ID.SelectedItem.Text
        Dim DISTRICT_TEXT As String = Me.txtDISTRICT_TEXT.Text.Trim
        ...
        'sql = "SELECT * From  CCARE_DICTINDEX_DISTRICT  Where   Id <>" & ViewState(ViewStateInfo.Object_Id) & " And upper(DISTRICT_TEXT)='" & DISTRICT_TEXT.ToUpper & "' "
        'If Me.B2CCheckData(sql) = False Then
        If ViewState(ViewStateInfo.Status) = Constants.Action.Insert Then
            retval = UpdateObject.B2CManagement.InsertDictIndexDistrict(PROVINCE_ID, PROVINCE_TEXT, DISTRICT_TEXT, STATUS_ID, STATUS_TEXT, CREATE_BY_ID, CREATE_BY_TEXT, CREATE_TIME, UPDATE_BY_ID, UPDATE_BY_TEXT, UPDATE_TIME, DESCRIPTION)
        ElseIf ViewState(ViewStateInfo.Status) = Constants.Action.Update Then
            retval = UpdateObject.B2CManagement.UpdateDictIndexDistrict(ViewState(ViewStateInfo.Object_Id), PROVINCE_ID, PROVINCE_TEXT, DISTRICT_TEXT, STATUS_ID, STATUS_TEXT, UPDATE_BY_ID, UPDATE_BY_TEXT, UPDATE_TIME, DESCRIPTION)
        End If
        If retval = "" Then
            RedirectUrl(Constants.Url._CcareB2C.CcareDictIndexDistrictList)
        Else
    ...
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDelete.Click
        Dim sql As String = "Delete  From  CCARE_DICTINDEX_DISTRICT  Where  Id =" & ViewState(ViewStateInfo.Object_Id)
        Try
