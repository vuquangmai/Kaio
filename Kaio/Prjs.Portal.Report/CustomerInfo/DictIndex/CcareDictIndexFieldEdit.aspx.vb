Public Class CcareDictIndexFieldEdit
    Inherits GlobalPage
#Region "Page Load"
...
#Region "Bind Data"
    Private Sub bindData(ByVal intId As Integer)
        Dim sql As String = "SELECT * FROM CCARE_DICTINDEX_FIELD Where Id=" & intId
        ...
        If dt.Rows.Count > 0 Then
            Me.txtFIELD_TEXT.Text = dt.Rows(0).Item("FIELD_TEXT")
...
#Region "Submit Click"
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnUpdate.Click
        If Me.txtFIELD_TEXT.Text = "" Then
            Me.lblerror.Text = "Nguồn dữ liệu không hợp lệ !"
            Exit Sub
        End If
        Dim FIELD_TEXT As String = Me.txtFIELD_TEXT.Text.Trim
        ...
        sql = "SELECT * From  CCARE_DICTINDEX_FIELD  Where   Id <>" & ViewState(ViewStateInfo.Object_Id) & " And upper(FIELD_TEXT)='" & FIELD_TEXT.ToUpper & "'"
        If Me.B2CCheckData(sql) = False Then
            If ViewState(ViewStateInfo.Status) = Constants.Action.Insert Then
                retval = UpdateObject.B2CManagement.InsertDictIndexField(FIELD_TEXT, STATUS_ID, STATUS_TEXT, CREATE_BY_ID, CREATE_BY_TEXT, CREATE_TIME, UPDATE_BY_ID, UPDATE_BY_TEXT, UPDATE_TIME, DESCRIPTION)
            ElseIf ViewState(ViewStateInfo.Status) = Constants.Action.Update Then
                retval = UpdateObject.B2CManagement.UpdateDictIndexField(ViewState(ViewStateInfo.Object_Id), FIELD_TEXT, STATUS_ID, STATUS_TEXT, UPDATE_BY_ID, UPDATE_BY_TEXT, UPDATE_TIME, DESCRIPTION)
            End If
            If retval = "" Then
    ...
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDelete.Click
        Dim sql As String = "Delete  From  CCARE_DICTINDEX_FIELD  Where  Id =" & ViewState(ViewStateInfo.Object_Id)
        Try
