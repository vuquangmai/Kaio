Public Class CcareDictIndexRefuseEdit
    Inherits GlobalPage
#Region "Page Load"
...
#Region "Bind Data"
    Private Sub bindData(ByVal intId As Integer)
        Dim sql As String = "SELECT * FROM CCARE_DICTINDEX_BLACKLIST Where Id=" & intId
        ...
        If dt.Rows.Count > 0 Then
            Me.txtUser_Id.Text = dt.Rows(0).Item("USER_ID")
...
#Region "Submit Click"
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnUpdate.Click
        ...
        sql = "SELECT * From  CCARE_DICTINDEX_BLACKLIST  Where   Id <>" & ViewState(ViewStateInfo.Object_Id) & " And upper(USER_ID)='" & USER_ID.ToUpper & "'"
        If Me.B2CCheckData(sql) = False Then
            If ViewState(ViewStateInfo.Status) = Constants.Action.Insert Then
                retval = UpdateObject.B2CManagement.InsertDictIndexBlackList(USER_ID, MOBILE_OPERATOR, CREATE_BY_ID, CREATE_BY_TEXT, CREATE_TIME, UPDATE_BY_ID, UPDATE_BY_TEXT, UPDATE_TIME, DESCRIPTION)
            ElseIf ViewState(ViewStateInfo.Status) = Constants.Action.Update Then
                retval = UpdateObject.B2CManagement.UpdateDictIndexBlackList(ViewState(ViewStateInfo.Object_Id), USER_ID, MOBILE_OPERATOR, UPDATE_BY_ID, UPDATE_BY_TEXT, UPDATE_TIME, DESCRIPTION)
            End If
            If retval = "" Then
                RedirectUrl(Constants.Url._CcareB2C.CcareDictIndexRefuseList)
            Else
    ...
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDelete.Click
        Dim sql As String = "Delete  From  CCARE_DICTINDEX_BLACKLIST  Where  Id =" & ViewState(ViewStateInfo.Object_Id)
        Try
