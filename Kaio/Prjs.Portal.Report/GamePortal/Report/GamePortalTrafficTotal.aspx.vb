...
Public Class GamePortalTrafficTotal
    Inherits GlobalPage
...
#Region "Page load"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Me.IsPostBack Then
            Me.lbltitle.Text = "BÁO CÁO DOANH THU DỊCH VỤ GAME PORTAL"
            BindDictIndex()
            InitData()
...
#Region "DictIndex"
    ...
    Private Sub BindServiceId()
        Dim sql As String = "SELECT  * FROM GamePortal_Channel_Service Order by Service_Text "
        Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
...
#Region "Init Data"
    Private Sub InitData()
        ...
        For Each item As Telerik.Web.UI.RadComboBoxItem In Me.RadDropDownListDayOfWeek.Items
            item.Checked = True
        Next
        For Each item As Telerik.Web.UI.RadComboBoxItem In Me.RadDropListChannel_Id.Items
            item.Checked = True
        Next
        For Each item As Telerik.Web.UI.RadComboBoxItem In Me.RadDropDownListService_Id.Items
            item.Checked = True
        Next
        For Each item As Telerik.Web.UI.RadComboBoxItem In Me.RadDropDownCooperation_Model_Id.Items
            item.Checked = True
        Next
    End Sub
...
#Region "Bind Data"
    Private Sub BindData(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        Dim vTable As String = "GamePortal_Data_" & Util.StringBuilder.ConvertDigit(Me.DropDownListYear.SelectedItem.Value) & Util.StringBuilder.ConvertDigit(Me.DropDownListMonth.SelectedItem.Value)
        ...
        sql = "SELECT substring(Date,1,4) as Year,substring(Date,5,2) as Month," & _
                              ...
                              " (case when '" & Me.CheckBoxCooperation_Model_Id.Checked & "'='False' then '--all--' else isnull(Cooperation_Model_Text,'Unknown') end ) as Cooperation_Model_Text," & _
        ...
        If Me.CheckBoxChannel_Id.Checked = True Then
            Dim CollectionChannelId As IList(Of Telerik.Web.UI.RadComboBoxItem) = Me.RadDropListChannel_Id.CheckedItems
            Dim sb As New StringBuilder()
            If CollectionChannelId.Count = 0 Then
                Me.lblerror.Text = "Kênh cung cấp game không hợp lệ !"
                Exit Sub
            Else
                If CollectionChannelId.Count < Me.RadDropListChannel_Id.Items.Count Then
        ...
        If Me.CheckBoxDayOfWeek.Checked = True Then
            ...
            End If
        End If
        If Me.CheckBoxCooperation_Model_Id.Checked = True Then
            Dim CollectionCooperation_Model As IList(Of Telerik.Web.UI.RadComboBoxItem) = Me.RadDropDownCooperation_Model_Id.CheckedItems
            Dim sb As New StringBuilder()
            If CollectionCooperation_Model.Count = 0 Then
                Me.lblerror.Text = "Thứ của tuần không hợp lệ !"
                Exit Sub
            Else
                If CollectionCooperation_Model.Count < Me.RadDropDownCooperation_Model_Id.Items.Count Then
                    For Each item As Telerik.Web.UI.RadComboBoxItem In CollectionCooperation_Model
                        If sb.ToString = "" Then
                            sb.Append("'" + item.Value + "'")
                        Else
                            sb.Append(",'" + item.Value + "'")
                        End If
                    Next
                    sqlConditional = sqlConditional & " And  Cooperation_Model_Id In (" & sb.ToString() & ")"
                End If
            End If
        End If
        ...
        End If
        If Me.CheckBoxCooperation_Model_Id.Checked = True Then
            sqlGroup = sqlGroup & ", Cooperation_Model_Text"
            sqlOrder = sqlOrder & ",Cooperation_Model_Text"
        End If
