Public Class KpiScratchCardDictIndexCcareEdit
    Inherits GlobalPage
#Region "Page Load"
        ...
        If Not Me.IsPostBack Then
            ...
            If IsNumeric(ViewState(ViewStateInfo.Object_Id)) Then
                If ViewState(ViewStateInfo.Object_Id) > 0 Then
                    Me.lbltitle.Text = "KPI DỊCH VỤ BRAND NAME - CẬP NHẬT TỪ ĐIỂN CSKH"
                    bindData(ViewState(ViewStateInfo.Object_Id))
                    ViewState(ViewStateInfo.Status) = Constants.Action.Update
                Else
                    Me.lbltitle.Text = "KPI DỊCH VỤ BRAND NAME - THÊM TỪ ĐIỂN CSKH"
...
#Region "Bind Data"
    Private Sub bindData(ByVal intId As Integer)
        Dim sql As String = "SELECT * FROM Kpi_ScratchCard_DictIndex_Ccare_Complain Where Id=" & intId
        ...
        If dt.Rows.Count > 0 Then
            ...
            Me.txtDecrease_Percent_Time.Text = IIf(IsDBNull(dt.Rows(0).Item("Decrease_Percent")) = True, "", dt.Rows(0).Item("Decrease_Percent"))
            Me.txtDecrease_Percent_Max_Time.Text = IIf(IsDBNull(dt.Rows(0).Item("Decrease_Percent_Max")) = True, "", dt.Rows(0).Item("Decrease_Percent_Max"))
...
#Region "Submit Click"
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnUpdate.Click
        ...
        Dim Decrease_Percent As Integer = Me.txtDecrease_Percent_Time.Text.Trim
        Dim Decrease_Percent_Max As Integer = Me.txtDecrease_Percent_Max_Time.Text.Trim
        ...
        Dim sql As String = ""
        sql = "SELECT * From  Kpi_ScratchCard_DictIndex_Ccare_Complain  Where   Id <>" & ViewState(ViewStateInfo.Object_Id) & " And Criteria_Text=N'" & Criteria_Text & "'"
        If Me.IsCheckData(sql) = False Then
            ...
            ElseIf ViewState(ViewStateInfo.Status) = Constants.Action.Update Then
                sql = "Update Kpi_ScratchCard_DictIndex_Ccare_Complain Set Standar_Threshold_Handle=N'" & Standar_Threshold_Handle & "'," & _
            ...
            Try
                ...
                RedirectUrl(Constants.Url._KPI.ScratchCard.KpiScratchCardDictIndexCcareList)
