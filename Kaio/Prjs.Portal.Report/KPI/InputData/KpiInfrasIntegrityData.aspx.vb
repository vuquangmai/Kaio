...
Public Class KpiInfrasIntegrityData
    Inherits GlobalPage
...
#Region "Page Load"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Me.IsPostBack Then
            Me.lbltitle.Text = "KPI HẠ TẦNG - TOÀN VẸN DỮ LIỆU"
            BindDictIndex()
            InitStatus()
            SetTitle()
            'CreateFolder()
...
#Region "DictIndex"
    ...
    Private Sub BindCriteriaIdQ1()
        Dim sql As String = "SELECT * FROM Kpi_Infras_DictIndex_Integrity_Data"
...
#Region "Bind Data"
    Private Sub BindDataQ1(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        Dim sql As String = "SELECT *,Row_Number() Over ( Order by Date_Text ) as RowNumber  FROM Kpi_Infras_Integrity_Data WHERE Date_Text='" & Util.DateTimeFomat.Timestamp2YYYYMMDD(Me.RadDate_IdQ1.SelectedDate.Value, "") & "'"
        ...
        ElseIf strAction = Constants.Action.Export Then
            ExportData.ExportExcel._KPI.Infras.IntegrityData(sql, CurrentUser.UserName)
        End If
    End Sub
#End Region
#Region "Pager"
...
#Region "Insert Data"
    Private Sub InsertDataQ1()
        ...
        Dim Recover_Data_Id As Integer = Me.DropDownListRecover_DataQ1.SelectedItem.Value
        Dim Recover_Data_Text As String = Me.DropDownListRecover_DataQ1.SelectedItem.Text
        ...
        Dim Decrease_Percent_NoRecover_Total As Integer = 0
        ...
        Dim sql As String = "SELECT *  FROM Kpi_Infras_DictIndex_Integrity_Data WHERE Criteria_Id=" & Criteria_Id
        Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
        If dt.Rows.Count > 0 Then
            ...
            Decrease_Percent_Quantity_Total = CaculateQualityIntegrityData(Standar_Threshold_Handle_Quantity, Standar_Threshold_Handle_Over_Quantity, Decrease_Percent_Quantity, Decrease_Percent_Max, 1) ' Luôn =0, cuối tháng tổng hợp b/c mới tính điểm KPI này
            Decrease_Percent_Time_Total = CaculateTimeIntegrityData(Standar_Threshold_Handle_Time, Standar_Threshold_Handle_Over_Time, Decrease_Percent_Max, Decrease_Percent_Time, Total_Min)
            Decrease_Percent_NoRecover_Total = IIf(Me.DropDownListRecover_DataQ1.SelectedItem.Value = 1, 0, dt.Rows(0).Item("Decrease_Percent_NoRecover"))
            Decrease_Percent_Total = Decrease_Percent_Quantity_Total + Decrease_Percent_Time_Total + Decrease_Percent_NoRecover_Total
        End If
        ...
        With cmd
            ...
            .CommandText = "KPI_Infras_Insert_Integrity_Data"
            ...
            .Parameters.Add(New SqlParameter("@Recover_Data_Id", SqlDbType.Int))
            .Parameters.Item("@Recover_Data_Id").Value = Recover_Data_Id

            .Parameters.Add(New SqlParameter("@Recover_Data_Text", SqlDbType.NVarChar, 500))
            .Parameters.Item("@Recover_Data_Text").Value = Recover_Data_Text
            ...
            .Parameters.Add(New SqlParameter("@Decrease_Percent_NoRecover_Total", SqlDbType.Int))
            .Parameters.Item("@Decrease_Percent_NoRecover_Total").Value = Decrease_Percent_NoRecover_Total
...
#Region "Update Data"
    Private Sub UpdateDataQ1()
        ...
        With cmd
            .Parameters.Clear()
            .Connection = GlobalConnection
            .CommandText = "KPI_Infras_Update_Integrity_Data"
            .CommandType = CommandType.StoredProcedure
...
#Region "Del Data"
    Sub DelQ1(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs)
        Dim sql As String = "Delete from Kpi_Infras_Integrity_Data Where Id=" & CType(e.CommandArgument, Integer)
        Try
...
#Region "Edit Data"
    Sub EditQ1(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs)
        ...
        Dim sql As String = "SELECT *  FROM Kpi_Infras_Integrity_Data Where Id=" & ViewState("Id_Q1")
        Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
        If dt.Rows.Count > 0 Then
...
#Region "Delete Data"
    Private Sub DelDataQ1()
        ...
        Next
        If vId <> "0" Then
            sql = "Delete From Kpi_Infras_Integrity_Data  WHERE Id IN (" & vId & ")"
            Try
                MSSQLEnv.ExecuteNonQuery.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
            Catch ex As Exception
...
#End Region
    ...
    End Function
    Private Function CaculateQualityIntegrityData(ByVal Standar_Threshold_Handle As Integer, _
        ...
        Return retval
    End Function
    Private Function CaculateTimeIntegrityData(ByVal Standar_Threshold_Handle As Integer, _
                                              ByVal Standar_Threshold_Handle_Over As Integer, _
                                              ByVal Decrease_Percent_Max As Integer, _
                                              ByVal Decrease_Percent As Integer, _
                                              ByVal Total_Min As Decimal) As Integer
