...
Public Class KpiInfrasSecurity
    Inherits GlobalPage
...
#Region "Page Load"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Me.IsPostBack Then
            Me.lbltitle.Text = "CHẤT LƯỢNG BẢO MẬT"
            BindDictIndex()
            InitStatus()
            SetTitle()
            'CreateFolder()
...
#Region "DictIndex"
    ...
    Private Sub BindCriteriaIdQ1()
        Dim sql As String = "SELECT * FROM Kpi_Infras_DictIndex_Security"
...
#Region "Bind Data"
    Private Sub BindDataQ1(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        Dim sql As String = "SELECT *,Row_Number() Over ( Order by Date_Text ) as RowNumber  FROM Kpi_Infras_Security WHERE Date_Text='" & Util.DateTimeFomat.Timestamp2YYYYMMDD(Me.RadDate_IdQ1.SelectedDate.Value, "") & "'"
        ...
        ElseIf strAction = Constants.Action.Export Then
            ExportData.ExportExcel._KPI.Infras.Security(sql, CurrentUser.UserName)
        End If
    End Sub
#End Region
#Region "Pager"
...
#Region "Insert Data"
    Private Sub InsertDataQ1()
        ...
        Dim sql As String = "SELECT *  FROM Kpi_Infras_DictIndex_Security WHERE Criteria_Id=" & Criteria_Id
        Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
        If dt.Rows.Count > 0 Then
            ...
            Decrease_Percent_Quantity_Total = CaculateQualitySecurity(Standar_Threshold_Handle_Quantity, Standar_Threshold_Handle_Over_Quantity, Decrease_Percent_Quantity, Decrease_Percent_Max, 0) ' Luôn =0, cuối tháng tổng hợp b/c mới tính điểm KPI này
            Decrease_Percent_Time_Total = CaculateTimeSecurity(Standar_Threshold_Handle_Time, Standar_Threshold_Handle_Over_Time, Standar_Threshold_Handle_Max_Time, Decrease_Percent_Max_Time, Decrease_Percent_Time, Total_Min)
        ...
        With cmd
            ...
            .CommandText = "KPI_Infras_Insert_Security"
...
#Region "Update Data"
    Private Sub UpdateDataQ1()
        ...
        If dt.Rows.Count > 0 Then
            ...
            Standar_Threshold_Handle_Max_Time = dt.Rows(0).Item("Standar_Threshold_Handle_Max_Time")
            Decrease_Percent_Max_Time = dt.Rows(0).Item("Decrease_Percent_Max_Time")
            Decrease_Percent_Quantity_Total = CaculateQualitySecurity(Standar_Threshold_Handle_Quantity, Standar_Threshold_Handle_Over_Quantity, Decrease_Percent_Quantity, Decrease_Percent_Max, 0) ' Luôn =0, cuối tháng tổng hợp b/c mới tính điểm KPI này(ngưỡng chuẩn là 0 lỗi)
            Decrease_Percent_Time_Total = CaculateTimeSecurity(Standar_Threshold_Handle_Time, Standar_Threshold_Handle_Over_Time, Standar_Threshold_Handle_Max_Time, Decrease_Percent_Max_Time, Decrease_Percent_Time, Total_Min)
            Decrease_Percent_Total = Decrease_Percent_Quantity_Total + Decrease_Percent_Time_Total
        End If
        ...
        With cmd
            .Parameters.Clear()
            .Connection = GlobalConnection
            .CommandText = "KPI_Infras_Update_Security"
            .CommandType = CommandType.StoredProcedure
...
#Region "Del Data"
    Sub DelQ1(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs)
        Dim sql As String = "Delete from Kpi_Infras_Security Where Id=" & CType(e.CommandArgument, Integer)
        Try
...
#Region "Edit Data"
    Sub EditQ1(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs)
        ...
        Dim sql As String = "SELECT *  FROM Kpi_Infras_Security Where Id=" & ViewState("Id_Q1")
        Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
        If dt.Rows.Count > 0 Then
...
#Region "Delete Data"
    Private Sub DelDataQ1()
        ...
        Next
        If vId <> "0" Then
            sql = "Delete From Kpi_Infras_Security  WHERE Id IN (" & vId & ")"
            Try
                MSSQLEnv.ExecuteNonQuery.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
            Catch ex As Exception
...
#End Region
    ...
    End Function
    Private Function CaculateQualitySecurity(ByVal Standar_Threshold_Handle As Integer, _
        ...
        Return retval
    End Function
    Private Function CaculateTimeSecurity(ByVal Standar_Threshold_Handle As Integer, _
                                              ByVal Standar_Threshold_Handle_Over As Integer, _
