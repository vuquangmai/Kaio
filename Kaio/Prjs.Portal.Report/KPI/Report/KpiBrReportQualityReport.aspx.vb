...
Public Class KpiBrReportQualityReport
    Inherits GlobalPage
...
#Region "Bind Data"
    Private Sub BindDataQ1(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        Dim sql As String = "SELECT *,Row_Number() Over ( Order by Date_Text ) as RowNumber  FROM Kpi_Br_Report_Quality WHERE Date_Text>='" & FromDate & "' AND Date_Text<='" & ToDate & "'"
        If txtTotal_Program_Error_Threshold_Q1.Text.Trim <> "" Then
            sql = sql & " AND Total_Program_Error_Threshold=" & Me.txtTotal_Program_Error_Threshold_Q1.Text.Trim
        End If
        If txtTotal_Program_Error_Q1.Text.Trim <> "" Then
            sql = sql & " AND Total_Program_Error=" & Me.txtTotal_Program_Error_Q1.Text.Trim
        End If
...
#Region "Bind Data"
    Private Sub BindDataQ2(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        Dim sqlTotal As String = ""
        Dim sql As String = "SELECT *,Row_Number() Over ( Order by Date_Text ) as RowNumber  FROM Kpi_Br_Report_Time WHERE Date_Text>='" & FromDate & "' AND Date_Text<='" & ToDate & "'"
        If strAction = Constants.Action.Search Then
            sqlTotal = "SELECT count(*) Total FROM (" & sql & ") T"
            sql = "SELECT * FROM (" & sql & ") T WHERE  T.RowNumber  >" & LowerBand & " and  T.RowNumber < " & UpperBand
            Dim dt As DataTable = Nothing
