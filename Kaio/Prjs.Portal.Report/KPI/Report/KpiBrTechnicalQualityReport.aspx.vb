...
Public Class KpiBrTechnicalQualityReport
    Inherits GlobalPage
...
#Region "DictIndex"
    ...
    Private Sub BindErrorCodeQ5()
        ...
        Me.DropDownListErrorCodeQ5.Items.Add(New ListItem("--all--", 0))
        If dt.Rows.Count > 0 Then
            For i As Integer = 0 To dt.Rows.Count - 1
...
#Region "Q1"
#Region "Ajax"
    Protected Sub RadDate_IdQ1_SelectedDateChanged(sender As Object, e As Telerik.Web.UI.Calendar.SelectedDateChangedEventArgs) Handles RadTime_EndQ1.SelectedDateChanged
...
#Region "Bind Data"
    Private Sub BindDataQ1(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        Dim sql As String = "SELECT *,Row_Number() Over ( Order by Date_Text ) as RowNumber  FROM Kpi_Br_Technical_Quality_MT_Telcos WHERE Date_Text>='" & FromDate & "' AND Date_Text<='" & ToDate & "'"
...
#Region "Bind Data"
    Private Sub BindDataQ2(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        Dim sqlTotal As String = ""
        Dim sql As String = "SELECT *,Row_Number() Over ( Order by Date_Text ) as RowNumber  FROM Kpi_Br_Technical_Quality_MT_Handset  WHERE Date_Text>='" & FromDate & "' AND Date_Text<='" & ToDate & "'"
        ...
        If Me.DropDownListUser_IdQ2.SelectedItem.Value > 0 Then
            sql = sql & " AND User_Id=" & Me.DropDownListUser_IdQ2.SelectedItem.Value
        End If
...
#Region "Bind Data"
    Private Sub BindDataQ3(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        Dim sqlTotal As String = ""
        Dim sql As String = "SELECT *,Row_Number() Over ( Order by Date_Text ) as RowNumber  FROM Kpi_Br_Technical_Quality_System_Error WHERE Date_Text>='" & FromDate & "' AND Date_Text<='" & ToDate & "'"
...
#Region "Bind Data"
    Private Sub BindDataQ5(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        Dim sqlTotal As String = ""
        Dim sql As String = "SELECT *,Row_Number() Over ( Order by Date_Text ) as RowNumber  FROM Kpi_Br_Input_Software_Error WHERE Date_Text>='" & FromDate & "' AND Date_Text<='" & ToDate & "'"
        If Me.txtProgram_CodeQ5.Text.Trim <> "" Then
            sql = sql & " AND Program_Code=N'" & Me.txtProgram_CodeQ5.Text.Trim & "'"
        End If
        If Me.DropDownListErrorCodeQ5.SelectedItem.Value > 0 Then
            sql = sql & " AND Error_Id=" & Me.DropDownListErrorCodeQ5.SelectedItem.Value
        End If
        If strAction = Constants.Action.Search Then
            sqlTotal = "SELECT count(*) Total FROM (" & sql & ") T"
            sql = "SELECT * FROM (" & sql & ") T WHERE  T.RowNumber  >" & LowerBand & " and  T.RowNumber < " & UpperBand
