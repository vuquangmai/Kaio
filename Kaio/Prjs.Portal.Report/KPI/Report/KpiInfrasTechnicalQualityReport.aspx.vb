...
Public Class KpiInfrasTechnicalQualityReport
    Inherits GlobalPage
...
#Region "Page Load"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Me.IsPostBack Then
            Me.lbltitle.Text = "BÁO CÁO CHẤT LƯỢNG KỸ THUẬT KPI HẠ TẦNG"
            BindDictIndex()
        End If
        LoadData()
    End Sub
...
#Region "DictIndex"
    ...
    Private Sub BindCriteriaIdQ3()
        ...
        Me.DropDownListCriteria_IdQ3.Items.Add(New ListItem("--all--", 0))
        If dt.Rows.Count > 0 Then
            For i As Integer = 0 To dt.Rows.Count - 1
...
#Region "Load Data"
    Private Sub LoadData()
        ...
        BindDataQ1(intPageSizeQ2, intCurentPageQ2, Constants.Action.Search)
        Dim intPageSizeQ3 As Integer = PagerQ3.PageSize
        Dim intCurentPageQ3 As Integer = PagerQ3.CurrentIndex
        BindDataQ1(intPageSizeQ3, intCurentPageQ3, Constants.Action.Search)
    End Sub
#End Region
#Region "Q1"
#Region "Bind Data"
    Private Sub BindDataQ1(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        Dim sql As String = "SELECT *,Row_Number() Over ( Order by Date_Text ) as RowNumber  FROM Kpi_Infras_Technical_Quality_System_Downtime WHERE Date_Text>='" & FromDate & "' AND Date_Text<='" & ToDate & "'"
...
#Region "Submit Click"
    ...
    Protected Sub btDelQ1_Click(sender As Object, e As EventArgs) Handles btSearchQ1.Click
        ViewState("DATA_GRID_Q1") = Nothing
        ViewState("DATA_COUNT_Q1") = Nothing
        Dim intPageSize As Integer = PagerQ1.PageSize
        Dim intCurentPage As Integer = PagerQ1.CurrentIndex
...
#Region "Bind Data"
    Private Sub BindDataQ2(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        Dim ToDate As String = Util.DateTimeFomat.Timestamp2YYYYMMDD(Me.RadTime_EndQ1.SelectedDate.Value, "")
        Dim sqlTotal As String = ""
        Dim sql As String = "SELECT *,Row_Number() Over ( Order by Date_Text ) as RowNumber  FROM Kpi_Infras_Technical_Quality_Server_Error WHERE Date_Text>='" & FromDate & "' AND Date_Text<='" & ToDate & "'"
...
#Region "Bind Data"
    Private Sub BindDataQ3(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        Dim ToDate As String = Util.DateTimeFomat.Timestamp2YYYYMMDD(Me.RadTime_EndQ1.SelectedDate.Value, "")
        Dim sqlTotal As String = ""
        Dim sql As String = "SELECT *,Row_Number() Over ( Order by Date_Text ) as RowNumber  FROM Kpi_Infras_Technical_Quality_Server_Performance WHERE Date_Text>='" & FromDate & "' AND Date_Text<='" & ToDate & "'"
        If Me.DropDownListCriteria_IdQ2.SelectedItem.Value > 0 Then
            sql = sql & " AND Criteria_Id=" & Me.DropDownListCriteria_IdQ2.SelectedItem.Value
        End If
        If strAction = Constants.Action.Search Then
