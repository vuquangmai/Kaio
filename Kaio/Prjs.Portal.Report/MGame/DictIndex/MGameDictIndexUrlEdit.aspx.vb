...
Public Class MGameDictIndexUrlEdit
    Inherits GlobalPage
#Region "Page Load"
...
#Region "Bind Data"
    Private Sub bindData(ByVal intId As Integer)
        Dim sql As String = "SELECT * FROM MGame_DictIndex_Url_Service Where Id=" & intId
        Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
        If dt.Rows.Count > 0 Then
...
#Region "Submit Click"
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnUpdate.Click
        ...
        Dim sql As String = ""
        sql = "SELECT * From  MGame_DictIndex_Url_Service  Where   Id <>" & ViewState(ViewStateInfo.Object_Id) & " And Url=N'" & Url & "'"
        If Me.IsCheckData(sql) = False Then
            If ViewState(ViewStateInfo.Status) = Constants.Action.Insert Then
                sql = "Insert Into MGame_DictIndex_Url_Service(Partner_Id, Url_Code,Ratio_Share, Status,Url, Create_Time, Create_By_Id, Create_By_Text, Update_Time, Update_By_Id,Update_By_Text,Description)" & _
                    "Values (N'" & Partner_Id & "',N'" & Url_Code & "',N'" & Ratio_Share & "',N'" & Status & "',N'" & Url & "',N'" & Create_Time & "',N'" & Create_By_Id & "',N'" & Create_By_Text & "',N'" & Update_Time & "',N'" & Update_By_Id & "',N'" & Update_By_Text & "',N'" & Description & "')"
            ElseIf ViewState(ViewStateInfo.Status) = Constants.Action.Update Then
                sql = "Update MGame_DictIndex_Url_Service Set Partner_Id=N'" & Partner_Id & "'," & _
     ...
     " Where Id=" & ViewState(ViewStateInfo.Object_Id)
            ...
            Try
                ...
                RedirectUrl(Constants.Url._MGame.MGameDictIndexUrlList)
    ...
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDelete.Click
        Dim sql As String = "Delete From MGame_DictIndex_Url_Service Where Id=" & ViewState(ViewStateInfo.Object_Id)
        Try
            MSSQLEnv.ExecuteNonQuery.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
            sql = "Delete From Ccare_Management_Contract_MGame Where Url_Id=" & ViewState(ViewStateInfo.Object_Id)
            MSSQLEnv.ExecuteNonQuery.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
