...
#Region "DictIndex"
    ...
    Private Sub BindPartnerd()
        Dim sql As String = "SELECT * FROM Vms_Appbox_Partners WHERE Status=1  Order by PartnerName"
        Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString("SqlConnectionString_56_1"), sql)
        Me.RadDropDownListPartner.Items.Clear()
        If dt.Rows.Count > 0 Then
            If dt.Rows.Count > 0 Then
                For i As Integer = 0 To dt.Rows.Count - 1
                    Me.RadDropDownListPartner.Items.Add(New Telerik.Web.UI.RadComboBoxItem(dt.Rows(i).Item("PartnerName"), dt.Rows(i).Item("Id")))
                Next
            End If
...
#Region "Bind Data"
    Private Sub BindData(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        Dim vTable As String = "MGame_Report"
        ...
        sql = "SELECT Year(NGAY) Year," & _
                ...
                " (case when '" & Me.CheckBoxPacket.Checked & "'='False' then '--all--' else (case convert(varchar,Loai_Goi) when '1' then N'Tải game' when '2' then N'Gói ngày' when   '3'  then N'Gói tuần' else 'Unknown' end)   end ) as Loai_Goi," & _
                ...
                " SUM(isnull(GIA_HAN_THAT_BAI,0)) GIA_HAN_THAT_BAI, " & _
                " SUM(isnull(TONG_GIA_HAN,0)) TONG_GIA_HAN, " & _
                " SUM(isnull(TONG_TIEN_TU_TAI_GAME,0)) TONG_TIEN_TU_TAI_GAME, " & _
                " SUM(isnull(TONG_TIEN_TU_GIA_HAN,0)) TONG_TIEN_TU_GIA_HAN, " & _
                " SUM(isnull(TONG_DOANH_THU,0)) TONG_DOANH_THU, " & _
                " SUM(isnull(TONG_HUY_LUY_KE,0)) TONG_HUY_LUY_KE, " & _
                " SUM(isnull(TONG_LUOT_TAI_GAME,0)) TONG_LUOT_TAI_GAME, " & _
            ...
            Else
                If CollectionPacket.Count < Me.RadDropDownListPacket.Items.Count Then
                    For Each item As Telerik.Web.UI.RadComboBoxItem In CollectionPacket
                        ...
                        End If
                    Next
                    sqlConditional = sqlConditional & " And  Loai_Goi In (" & sb.ToString() & ")"
                End If
            End If
        End If
        ...
        End If
        If Me.CheckBoxPacket.Checked = True Then
            sqlGroup = sqlGroup & ", Loai_Goi"
        End If
        sql = sql & sqlConditional & sqlGroup

        If strAction = Constants.Action.Search Then
            Dim dtPageCount As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString("SqlConnectionString_56_1"), "SELECT count(*) Total FROM (" & sql & ") T")
