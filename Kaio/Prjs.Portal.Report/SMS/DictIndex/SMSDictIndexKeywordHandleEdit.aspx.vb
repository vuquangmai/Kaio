...
Public Class SMSDictIndexKeywordHandleEdit
    Inherits GlobalPage
...
#Region "Page Load"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Me.IsPostBack Then
            Me.lbltitle.Text = "XỬ LÝ KHAI BÁO SỬA MÃ DỊCH VỤ SMS"
            BindDictIndex()
...
#Region "DictIndex"
    ...
    Private Sub BindCate1(ByVal intRootId As Integer)
        ...
        Me.DropDownListCate1_EditQ8.Items.Clear()
        Me.DropDownListCate1_EditQ8.Items.Add(New ListItem(("--Chọn--"), 0))
        If dt.Rows.Count > 0 Then
            For i As Integer = 0 To dt.Rows.Count - 1
                Me.DropDownListCate_1Q8.Items.Add(New ListItem(dt.Rows(i).Item("Cate_Name"), dt.Rows(i).Item("Id")))
                Me.DropDownListCate1_EditQ8.Items.Add(New ListItem(dt.Rows(i).Item("Cate_Name"), dt.Rows(i).Item("Id")))
            Next
        End If
    End Sub
...
#Region "Bind Grid"
    Private Sub BindQ1Data(ByVal strAction As String)
        ...
        sql = "SELECT *,row_number() over( Order by Id desc) as RowNumber  FROM " & vTable & " WHERE TypeOf_Ticket_Id=2 AND Status_Id=" & Constants.DeclareKeyword.StatusId.Create_New
...
#Region "Bind Grid"
    Private Sub BindQ2Data(ByVal strAction As String)
        ...
        sql = "SELECT *,row_number() over( Order by Id) as RowNumber  FROM " & vTable & " WHERE TypeOf_Ticket_Id=2 AND Status_Id=" & Constants.DeclareKeyword.StatusId.Request_Routing
        sql = sql & " AND Group_Handle_Id  IN (SELECT Group_Id  FROM SMS_DictIndex_Keyword_Declare_User_Handle WHERE User_Id=" & CurrentUser.UserId & ")"
...
#Region "Bind Grid"
    Private Sub BindQ3Data(ByVal strAction As String)
        ...
        sql = "SELECT *,row_number() over( Order by Id) as RowNumber  FROM " & vTable & " WHERE TypeOf_Ticket_Id=2 AND Status_Id=" & Constants.DeclareKeyword.StatusId.Declare_Routing
        sql = sql & " AND Group_Handle_Id  IN (SELECT Group_Id  FROM SMS_DictIndex_Keyword_Declare_User_Handle WHERE User_Id=" & CurrentUser.UserId & ")"
...
#Region "Bind Grid"
    Private Sub BindQ5Data(ByVal strAction As String)
        ...
        sql = "SELECT *,row_number() over( Order by Id) as RowNumber  FROM " & vTable & " WHERE TypeOf_Ticket_Id=2 AND Status_Id=" & Constants.DeclareKeyword.StatusId.Declare_Telcos
        sql = sql & " AND Group_Handle_Id  IN (SELECT Group_Id  FROM SMS_DictIndex_Keyword_Declare_User_Handle WHERE User_Id=" & CurrentUser.UserId & ")"
...
#Region "Bind Grid"
    Private Sub BindQ6Data(ByVal strAction As String)
        ...
        sql = "SELECT *,row_number() over( Order by Id) as RowNumber  FROM " & vTable & " WHERE TypeOf_Ticket_Id=2 AND Status_Id=" & Constants.DeclareKeyword.StatusId.Declare_Report
        sql = sql & " AND Group_Handle_Id  IN (SELECT Group_Id  FROM SMS_DictIndex_Keyword_Declare_User_Handle WHERE User_Id=" & CurrentUser.UserId & ")"
...
#Region "Bind Grid"
    Private Sub BindQ7Data(ByVal strAction As String)
        ...
        sql = "SELECT *,row_number() over( Order by Id) as RowNumber  FROM " & vTable & " WHERE TypeOf_Ticket_Id=2 AND Status_Id=" & Constants.DeclareKeyword.StatusId.Add_Partner
        sql = sql & " AND Group_Handle_Id  IN (SELECT Group_Id  FROM SMS_DictIndex_Keyword_Declare_User_Handle WHERE User_Id=" & CurrentUser.UserId & ")"
...
#Region "Q8 Event"
    Private Sub BindDataQ8(ByVal intId As Integer)
        ...
        If dt.Rows.Count > 0 Then
            ...
            Me.txtRouting_Url_EditQ8.Text = dt.Rows(0).Item("Routing_Url_Edit")
            Me.txtMTQ8.Text = dt.Rows(0).Item("MT")
            Me.txtMT_EditQ8.Text = dt.Rows(0).Item("MT_Edit")
            Me.txtTotal_MTQ8.Text = dt.Rows(0).Item("Total_MT")
            Me.txtTotal_MT_EditQ8.Text = dt.Rows(0).Item("Total_MT_Edit")
            Me.txtService_TextQ8.Text = dt.Rows(0).Item("Service_Text")
            Me.txtService_Text_EditQ8.Text = dt.Rows(0).Item("Service_Text_Edit")
            Me.txtService_DescriptionQ8.Text = dt.Rows(0).Item("Service_Description")
            Me.txtService_Description_EditQ8.Text = dt.Rows(0).Item("Service_Description_Edit")
            Me.lblUserFileQ8.Text = " <a href=" & dt.Rows(0).Item("Url_File") & " target=""_blank""   title =""Download"" >" & dt.Rows(0).Item("Url_File") & "</a>"
            Me.lblUserFile_EditQ8.Text = " <a href=" & dt.Rows(0).Item("Url_File_Edit") & " target=""_blank""   title =""Download"" >" & dt.Rows(0).Item("Url_File_Edit") & "</a>"
            ...
            Me.DropDownListCate1_EditQ8.SelectedValue = dt.Rows(0).Item("Cate1_Id_Edit")
