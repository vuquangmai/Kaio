...
Public Class SMS9977ViettelUserPacket
    Inherits GlobalPage
...
#Region "Page Load"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Me.IsPostBack Then
            Me.lblTitle.Text = "DANH SÁCH KHÁCH HÀNG ĐĂNG KÝ SUBS"
            BindDate()
...
#Region "Bind Data"
    Private Sub bindData(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        Dim vTable As String = "user_packet"
        ...
        Dim sql As String = "SELECT A.*, B.register_date  FROM user_packet A INNER JOIN user B  WHERE date_format(B.register_date,'%Y%m%d')>='" & FromDate & "'"
        sql = sql & " AND date_format(B.register_date,'%Y%m%d')<='" & ToDate & "'"
        If Me.txtMsisdn.Text.Trim <> "" Then
            sql = sql & " AND msisdn ='" & Me.txtMsisdn.Text.Trim & "'"
        End If
        If Me.txtPacket_Code.Text.Trim <> "" Then
            sql = sql & " AND packet_code = '" & Me.txtPacket_Code.Text.Trim & "'"
        End If
