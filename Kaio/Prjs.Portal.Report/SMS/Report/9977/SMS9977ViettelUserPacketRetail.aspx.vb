...
Public Class SMS9977ViettelUserPacketRetail
    Inherits GlobalPage
...
#Region "Page Load"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Me.IsPostBack Then
            Me.lblTitle.Text = "DANH SÁCH KHÁCH HÀNG MUA LẺ"
            BindDate()
...
#Region "Bind Data"
    Private Sub bindData(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        Dim sql As String = "SELECT * FROM user_packet_retail  WHERE date_format(date_create,'%Y%m%d')>='" & FromDate & "'"
        sql = sql & " AND date_format(date_create,'%Y%m%d')<='" & ToDate & "'"
        If Me.txtMsisdn.Text.Trim <> "" Then
            sql = sql & " AND msisdn ='" & Me.txtMsisdn.Text.Trim & "'"
        End If
        If Me.txtPacket_Code.Text.Trim <> "" Then
            sql = sql & " AND packet_id = '" & Me.txtPacket_Code.Text.Trim & "'"
        End If
