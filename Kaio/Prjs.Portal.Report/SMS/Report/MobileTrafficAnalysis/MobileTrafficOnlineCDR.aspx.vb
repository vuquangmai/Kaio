Public Class MobileTrafficOnlineCDR
    Inherits GlobalPage
...
#Region "Page load"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Me.IsPostBack Then
            Me.lblTitle.Text = "TRA CỨU LOG GIAO DỊCH CDR"
            BindDictIndex()
            InitData()
...
#Region "Bind Data"
    Private Function BuildDataTable(ByVal j As Integer) As String
        ...
        Dim vTable As String = "cdr_log" & Util.StringBuilder.ConvertDigit(Me.DropDownListYear.SelectedItem.Value) & Util.StringBuilder.ConvertDigit(Me.DropDownListMonth.SelectedItem.Value)
        ...
        If DropDownListStatus.SelectedItem.Value > -1 Then
            If Me.DropDownListStatus.SelectedItem.Value = 1 Then
                sqlConditional = sqlConditional & " AND process_result=" & Me.DropDownListStatus.SelectedItem.Value
            Else
                sqlConditional = sqlConditional & " AND process_result<>1"
            End If
        End If
        ...
        If Me.txtUser_Id.Text.Trim <> "" Then
            ...
            ElseIf User_id.StartsWith(84) = False Then
                User_id = "84" & User_id
            End If
            sqlConditional = sqlConditional & " AND user_id='" & User_id & "'"
        End If
        sql = "SELECT id, operator, service_id, user_id, send_time, deliver_time, keyword, content_type, (CASE message_type WHEN '1' THEN 'Charging' WHEN '2' THEN 'ReFund'  WHEN '0' THEN 'No Charge' ELSE 'Unknown' END) message_type, number_message, request_id, thirdparty, autotimestamp,if(process_result=1,'Success','Error') status  , money, info,DATE_FORMAT(loggingTime,'%Y-%m-%d %H:%i:%s') loggingTime " & _
