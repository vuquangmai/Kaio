Public Class MobileTrafficOnlineMO
    Inherits GlobalPage
...
#Region "Page load"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Me.IsPostBack Then
            Me.lblTitle.Text = "TRA CỨU LOG GIAO DỊCH MO"
            BindDictIndex()
            InitData()
...
#Region "Bind Data"
    Private Function BuildDataTable(ByVal j As Integer) As String
        ...
        Dim vTable As String = "mo" & Util.StringBuilder.ConvertDigit(Me.DropDownListYear.SelectedItem.Value) & Util.StringBuilder.ConvertDigit(Me.DropDownListMonth.SelectedItem.Value)
        ...
        Else
            If CollectionShortCode.Count < Me.RadDropDownListShortCode.Items.Count Then
                For Each item As Telerik.Web.UI.RadComboBoxItem In CollectionShortCode
                    ...
                    End If
                Next
                sqlConditional = sqlConditional & " AND  receiver In (" & sb.ToString() & ")"
            End If
        End If
        ...
        If Me.txtUser_Id.Text.Trim <> "" Then
            ...
            ElseIf User_id.StartsWith(84) = False Then
                User_id = "84" & User_id
            End If
            sqlConditional = sqlConditional & " AND sender='" & User_id & "'"
        End If
        'id, sender, receiver, msgdata, udhdata, receive_time, deliver_time, keyword, service_uri, thirdparty, operator, gateway, node, ip, app_name, status, error_no, error_message, request_id, autotimestamp, notes, money, loggingTime
        sql = "SELECT id, sender, receiver, msgdata, udhdata, receive_time, deliver_time, keyword, service_uri, thirdparty, operator, gateway, node, ip, app_name, if(status=0,'Success',concat('Error:',concat('<font color=red>',if(error_message is null,'',error_message), if(notes is null,'',notes),'</font>' ))) status , error_no, error_message, request_id, autotimestamp, notes, money, DATE_FORMAT(loggingTime,'%Y-%m-%d %H:%i:%s') loggingTime " & _
