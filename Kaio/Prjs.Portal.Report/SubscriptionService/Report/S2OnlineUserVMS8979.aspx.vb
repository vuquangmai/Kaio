...
Public Class S2OnlineUserVMS8979
    Inherits GlobalPage
...
#Region "Page load"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Me.IsPostBack Then
            Me.lbltitle.Text = "BÁO CÁO ĐĂNG KÝ, HỦY DỊCH VỤ 8979 - VMS"
            BindDictIndex()
...
#Region "Bind Data"
    Private Sub BindData(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        Dim vTable As String = "S2_TTKD_RegisteredUsers"
        ...
        Dim vParaTime As String = IIf(Me.DropDownListStatus.SelectedItem.Value = 1, "RegisteredTime", "CancelTime")

        sql = "SELECT  YEAR(A." & vParaTime & ") as vYear , " & _
              " (case when '" & Me.CheckBoxMonth.Checked & "'='False' then '--all--' else  convert(varchar,month(A." & vParaTime & ")) end) as vMonth," & _
              " (case when '" & Me.CheckBoxDate.Checked & "'='False' then '--all--' else  DATENAME(day,A." & vParaTime & ") end ) as vDate," & _
              " (case when '" & Me.CheckBoxHour.Checked & "'='False' then '--all--' else  DATENAME(hour, A." & vParaTime & ") end ) as vHour, " & _
              " (case when '" & Me.CheckBoxUser_Id.Checked & "'='False' then '--all--' else A.MSISDN end ) as User_Id, " & _
              ...
              " COUNT(*) Total,  row_number() over( Order by YEAR(A." & vParaTime & ") desc) as RowNumber " & _
              " FROM " & vTable & " A INNER JOIN S2_TTKD_Services B on A.Service_ID=B.Service_ID " & _
              " LEFT JOIN S2_TTND_Partners C on B.PartnerID =C.PartnerID "

        sqlTotal = "SELECT  YEAR(A." & vParaTime & ") as vYear , " & _
              " (case when '" & Me.CheckBoxMonth.Checked & "'='False' then '--all--' else  convert(varchar,month(A." & vParaTime & ")) end) as vMonth," & _
              " (case when '" & Me.CheckBoxDate.Checked & "'='False' then '--all--' else  DATENAME(day,A." & vParaTime & ") end ) as vDate," & _
              " (case when '" & Me.CheckBoxHour.Checked & "'='False' then '--all--' else  DATENAME(hour, A." & vParaTime & ") end ) as vHour, " & _
              " (case when '" & Me.CheckBoxUser_Id.Checked & "'='False' then '--all--' else A.MSISDN end ) as User_Id, " & _
        ...
        sqlConditional = " WHERE A.Service_Type = 'VIOLET' and A.Status=" & Me.DropDownListStatus.SelectedItem.Value
        If Me.CheckBoxAllDate.Checked = False Then
            sqlConditional = sqlConditional & " AND CONVERT(varchar, A." & vParaTime & ",112) >='" & vFromDate & "' AND CONVERT(varchar ,A." & vParaTime & ",112)<='" & vToDate & "'"
        End If
