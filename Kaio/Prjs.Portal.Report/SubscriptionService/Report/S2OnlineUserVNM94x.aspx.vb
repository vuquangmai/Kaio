...
Public Class S2OnlineUserVNM94x
    Inherits GlobalPage
...
#Region "Page load"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Me.IsPostBack Then
            Me.lbltitle.Text = "BÁO CÁO ĐĂNG KÝ, HỦY DỊCH VỤ 94x - V//"
            BindDictIndex()
...
#Region "DictIndex"
    ...
    Private Sub BindServiceId()
        Dim sql As String = "SELECT * FROM S2_TTND_Subscription_Services where Service_Type='1006' "
        Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString("SqlConnectionString_168_2"), sql)
...
#Region "Bind Data"
    Private Sub BindData(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        sql = "SELECT  YEAR(A.RegisteredTime) as vYear , " & _
              " (case when '" & Me.CheckBoxMonth.Checked & "'='False' then '--all--' else  convert(varchar,month(A.RegisteredTime)) end) as vMonth," & _
              " (case when '" & Me.CheckBoxDate.Checked & "'='False' then '--all--' else  DATENAME(day,A.RegisteredTime) end ) as vDate," & _
              " (case when '" & Me.CheckBoxHour.Checked & "'='False' then '--all--' else  DATENAME(hour, A.RegisteredTime) end ) as vHour, " & _
              ...
              " (case when '" & Me.CheckBoxChannel.Checked & "'='False' then '--all--' else convert(nvarchar,A.Registration_Channel)  end ) as vRegistration_Channel,  " & _
              " (case when '" & Me.CheckBoxPartnerId.Checked & "'='False' then '--all--' else convert(nvarchar,C.PartnerName) end ) as vPartnerName," & _
              " COUNT(*) Total,  row_number() over( Order by YEAR(A.RegisteredTime) desc) as RowNumber " & _
              " FROM " & vTable & " A LEFT JOIN S2_TTND_Subscription_Services B on A.Service_ID=B.Id " & _
              " LEFT JOIN S2_TTND_Partners C on B.PartnerID =C.PartnerID "

        sqlTotal = "SELECT  YEAR(A.RegisteredTime) as vYear , " & _
              " (case when '" & Me.CheckBoxMonth.Checked & "'='False' then '--all--' else  convert(varchar,month(A.RegisteredTime)) end) as vMonth," & _
              " (case when '" & Me.CheckBoxDate.Checked & "'='False' then '--all--' else  DATENAME(day,A.RegisteredTime) end ) as vDate," & _
              " (case when '" & Me.CheckBoxHour.Checked & "'='False' then '--all--' else  DATENAME(hour, A.RegisteredTime) end ) as vHour, " & _
              " (case when '" & Me.CheckBoxUser_Id.Checked & "'='False' then '--all--' else A.User_id end ) as User_Id, " & _
        ...
        sqlConditional = " WHERE B.Service_Type='1006'"
        If Me.CheckBoxAllDate.Checked = False Then
            sqlConditional = sqlConditional & " AND CONVERT(varchar, A.RegisteredTime,112) >='" & vFromDate & "' AND CONVERT(varchar ,A.RegisteredTime,112)<='" & vToDate & "'"
        End If
            ...
            Else
                If CollectionChannel.Count < Me.RadDropDownListChannel.Items.Count Then
                    For Each item As Telerik.Web.UI.RadComboBoxItem In CollectionChannel
                        ...
                        End If
                    Next
                    sqlConditional = sqlConditional & " And  A.Registration_Channel In (" & sb.ToString() & ")"
                End If
            End If
        End If


        sqlGroup = " GROUP BY YEAR(A.RegisteredTime)"
        If Me.CheckBoxPartnerId.Checked Then
            sqlGroup = sqlGroup & ", C.PartnerName"
        End If
        If Me.CheckBoxMonth.Checked Then
            sqlGroup = sqlGroup & ",CONVERT(varchar,month(A.RegisteredTime)) "
            sqlOrder = sqlOrder & ",CAST(month as INT)"
        End If
        If Me.CheckBoxDate.Checked Then
            sqlGroup = sqlGroup & ", DATENAME(day,A.RegisteredTime)"
            sqlOrder = sqlOrder & ",CAST(day as INT)"
        End If
        If Me.CheckBoxHour.Checked Then
            sqlGroup = sqlGroup & ",  DATENAME(hour, A.RegisteredTime) "
