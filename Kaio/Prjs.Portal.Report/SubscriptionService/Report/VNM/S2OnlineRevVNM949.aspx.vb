...
Public Class S2OnlineRevVNM949
    Inherits GlobalPage
...
#Region "Page load"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Me.IsPostBack Then
            Me.lbltitle.Text = "BÁO CÁO DOANH THU TỰ DOANH ĐẦU SỐ 949 (ACCOUNT 2G) - V//"
            BindDictIndex()
            InitData()
...
#Region "Bind Data"
    Private Sub BindData(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
      ...
      " A.Short_Code as vShortcode,  convert(nvarchar,B.Service_Name) as vServices, convert(nvarchar,C.PartnerName) as vPartnerName ,Charging_Account," & _
        ...
        " WHEN '500' THEN cast(round(convert(int,196)*1,0) as decimal(18,0)) " & _
        " WHEN '1000' THEN cast(round(convert(int,391)*1,0) as decimal(18,0)) " & _
        " WHEN '2000' THEN cast(round(convert(int,782)*1,0) as decimal(18,0)) " & _
        " WHEN '3000' THEN cast(round(convert(int,1173)*1,0) as decimal(18,0)) " & _
        " WHEN '4000' THEN cast(round(convert(int,1563)*1,0) as decimal(18,0)) " & _
        " WHEN '5000' THEN cast(round(convert(int,1954)*1,0) as decimal(18,0)) " & _
        " WHEN '10000' THEN cast(round(convert(int,3909 )*1,0) as decimal(18,0)) " & _
        " WHEN '15000' THEN cast(round(convert(int,5863)*1,0) as decimal(18,0)) " & _
        " else  '0' end, " & _
        " vMoneyTotalVNM= case A.Charging_Price  " & _
        " WHEN '500' THEN cast(round(convert(int,454-196)*1,0) as decimal(18,0)) " & _
        " WHEN '1000' THEN cast(round(convert(int,909-391)*1,0) as decimal(18,0)) " & _
        " WHEN '2000' THEN cast(round(convert(int,1818-782)*1,0) as decimal(18,0)) " & _
        " WHEN '3000' THEN cast(round(convert(int,2727-1173)*1,0) as decimal(18,0)) " & _
        " WHEN '4000' THEN cast(round(convert(int,3636-1563)*1,0) as decimal(18,0)) " & _
        " WHEN '5000' THEN cast(round(convert(int,4545-1954)*1,0) as decimal(18,0)) " & _
        " WHEN '10000' THEN cast(round(convert(int,9090-3909 )*1,0) as decimal(18,0)) " & _
        " WHEN '15000' THEN cast(round(convert(int,13636-5863)*1,0) as decimal(18,0)) " & _
        " else  '0' end,  " & _
       ...
       IIf(Me.RadDropDownListStatus.SelectedItem.Value = 1, "  WHERE B.Service_Type='1001'  AND Charging_Status=1 ", " WHERE B.Service_Type='1001'  AND Charging_Status=0")
        ...
        sqlGroup = " GROUP BY  vYear,vMonth,Charging_Account"
        sqlOrder = " Order by  vYear,vMonth,Charging_Account "
