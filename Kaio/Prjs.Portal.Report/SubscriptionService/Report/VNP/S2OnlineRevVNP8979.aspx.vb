...
Public Class S2OnlineRevVNP8979
    Inherits GlobalPage
...
#Region "Page load"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Me.IsPostBack Then
            Me.lbltitle.Text = "BÁO CÁO DOANH THU ĐẦU SỐ 8979 - VINAPHONE"
            BindDictIndex()
            InitData()
...
#Region "Bind Data"
    Private Sub BindData(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        Dim vTable As String = "S2_VNP_Total_" & Util.StringBuilder.ConvertDigit(Me.DropDownListYear.SelectedItem.Value) & Util.StringBuilder.ConvertDigit(Me.DropDownListMonth.SelectedItem.Value)
        ...
        sql = "SELECT  substring(A.Date,1,4) as vYear ,substring(A.Date,5,2)  as vMonth," & _
            ...
            " (case when '" & Me.CheckBoxPrice_Unit.Checked & "'='False' then '--all--' else convert(nvarchar,A.Fee) end ) as vPrice ," & _
            " (case when '" & Me.CheckBoxPartnerId.Checked & "'='False' then '--all--' else convert(nvarchar,C.PartnerName) end ) as vPartnerName," & _
            " SUM(Cdr) vTotal,  SUM(cast(Revenue_Total as decimal))  vMoneyTotal, SUM(cast(Revenue_Share as decimal))  vMoneyVMG,(SUM(cast(Revenue_Total as decimal))-SUM(cast(Revenue_Share as decimal)))  vMoneyTelcos," & _
            " row_number() over( Order by  SUM(cast(Revenue_Share as decimal))  desc) as RowNumber " & _
            "  FROM " & _
              vTable & " A  LEFT JOIN S2_GPC_TTND_Services B ON   A.ServiceID COLLATE DATABASE_DEFAULT =B.Service_ID COLLATE DATABASE_DEFAULT " & _
        ...
        If Me.CheckBoxPartnerId.Checked And Me.DropDownListPartner_Id.SelectedItem.Value > 0 Then
            sqlConditional = sqlConditional & " and convert(nvarchar,C.PartnerName)=N'" & Me.DropDownListPartner_Id.SelectedItem.Text & "'"
        End If
        If Me.CheckBoxService_Id.Checked And Me.DropDownListService_Id.SelectedItem.Value > 0 Then
            sqlConditional = sqlConditional & " and A.SPID+ A.ServiceID='" & Me.DropDownListService_Id.SelectedItem.Value & "'"
        End If
        ...
        If strAction = Constants.Action.Search Then
            sqlTotal = "SELECT SUM(vTotal) vTotalCDR,SUM(vMoneyVMG) vMoneyVMG,SUM(vMoneyTelcos) vMoneyTelcos, SUM(vMoneyTotal)  as vMoneyTotal, COUNT(*)  TotalRecord FROM  (" & sql & ") T1 "
            ...
            If dt.Rows.Count > 0 Then
                ...
                Me.lblMoney_Telcos_VMG.Text = Utils.FormatDecimal(dtPageCount.Rows(0).Item("vMoneyTelcos"), 0)
