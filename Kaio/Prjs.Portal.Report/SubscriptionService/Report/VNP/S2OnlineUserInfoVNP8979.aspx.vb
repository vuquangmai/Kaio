...
Public Class S2OnlineUserInfoVNP8979
    Inherits GlobalPage
...
#Region "Bind Data"
    Private Sub bindDataUser()
        ...
        " FROM S2_GPC_TTND_RegisteredUsers A LEFT JOIN  S2_GPC_TTND_Services B on A.Service_ID=B.Service_ID " & _
    ...
    Private Sub BindData(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        sql = " SELECT A.User_Id Msisdn,A.Message,A.Short_Code ,A.Response_Time,B.service_name,convert(nvarchar,C.PartnerName) PartnerName " & _
                " FROM " & vTable & " A LEFT JOIN S2_GPC_TTND_Services B on A.Service_ID=B.Service_ID " & _
                " LEFT JOIN S2_TTND_Partners C on B.PartnerID =C.PartnerID " & _
                " WHERE  A.User_Id='" & Me.txtUser_Id.Text.Trim & "'"

        sql = sql & " AND CONVERT(varchar,A.Response_Time,112) >='" & vFromDate & "'  AND CONVERT(varchar,A.Response_Time,112)<='" & vToDate & "'"
