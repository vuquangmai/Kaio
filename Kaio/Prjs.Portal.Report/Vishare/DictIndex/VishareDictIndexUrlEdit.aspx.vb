...
Public Class VishareDictIndexUrlEdit
    Inherits GlobalPage
#Region "Page Load"
        ...
        If Not Me.IsPostBack Then
            ...
            If IsNumeric(ViewState(ViewStateInfo.Object_Id)) Then
                If ViewState(ViewStateInfo.Object_Id) > 0 Then
                    Me.lbltitle.Text = "SỬA ĐỔI THÔNG TIN URL TRUYỀN THÔNG"
                    bindData(ViewState(ViewStateInfo.Object_Id))
                    ViewState(ViewStateInfo.Status) = Constants.Action.Update
                Else
                    Me.lbltitle.Text = "THÊM THÔNG TIN URL TRUYỀN THÔNG"
...
#Region "DictIndex"
    Private Sub BindPartner()
        Dim sql As String = "SELECT * FROM Ccare_Management_Partner A INNER JOIN Ccare_Management_Contract B " & _
                                      " ON A.Id=B.Partner_Id WHERE B.Service_Id=" & Constants.ServiceInfo.Id.Vishare & " Order by Confidence_Text"
...
#Region "Bind Data"
    Private Sub bindData(ByVal intId As Integer)
        Dim sql As String = "SELECT * FROM Vishare_DictIndex_Url_Service Where Id=" & intId
        Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
        If dt.Rows.Count > 0 Then
            ...
            Me.txtRatio_Share_Vishare.Text = IIf(IsDBNull(dt.Rows(0).Item("Ratio_Share_Vishare")) = True, "", dt.Rows(0).Item("Ratio_Share_Vishare"))
            Me.txtRatio_Share_Clip.Text = IIf(IsDBNull(dt.Rows(0).Item("Ratio_Share_Clip")) = True, "", dt.Rows(0).Item("Ratio_Share_Clip"))
            Me.txtRatio_Share_Anh.Text = IIf(IsDBNull(dt.Rows(0).Item("Ratio_Share_Anh")) = True, "", dt.Rows(0).Item("Ratio_Share_Anh"))
...
#Region "Submit Click"
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnUpdate.Click
        ...
        If Me.txtUrl.Text = "" Then
            ...
            Exit Sub
        End If
        If Me.txtRatio_Share_Vishare.Text = "" Or Not IsNumeric(Me.txtRatio_Share_Vishare.Text.Trim) Then
            Me.lblerror.Text = "Tỷ lê gói Vishare không hợp lệ !"
            Exit Sub
        End If
        If Me.txtRatio_Share_Clip.Text.Trim = "" Or Not IsNumeric(Me.txtRatio_Share_Clip.Text.Trim) Then
            Me.lblerror.Text = "Tỷ lê gói Clip không hợp lệ !"
            Exit Sub
        End If
        If Me.txtRatio_Share_Anh.Text = "" Or Not IsNumeric(Me.txtRatio_Share_Anh.Text.Trim) Then
            Me.lblerror.Text = "Tỷ lê gói Ảnh không hợp lệ !"
            Exit Sub
        End If
        Dim Partner_Id As Integer = Me.DropDownListPartner_Id.SelectedItem.Value
        Dim Url_Code As String = Me.txtUrl_Code.Text.Trim
        Dim Ratio_Share_Vishare As String = Me.txtRatio_Share_Vishare.Text.Trim
        Dim Ratio_Share_Clip As String = Me.txtRatio_Share_Clip.Text.Trim
        Dim Ratio_Share_Anh As String = Me.txtRatio_Share_Anh.Text.Trim
        ...
        Dim sql As String = ""
        sql = "SELECT * From  Vishare_DictIndex_Url_Service  Where   Id <>" & ViewState(ViewStateInfo.Object_Id) & " And Url=N'" & Url & "'"
        If Me.IsCheckData(sql) = False Then
            If ViewState(ViewStateInfo.Status) = Constants.Action.Insert Then
                sql = "Insert Into Vishare_DictIndex_Url_Service(Partner_Id, Url_Code,Ratio_Share_Vishare,Ratio_Share_Clip,Ratio_Share_Anh, Status,Url, Create_Time, Create_By_Id, Create_By_Text, Update_Time, Update_By_Id,Update_By_Text,Description)" & _
                    "Values (N'" & Partner_Id & "',N'" & Url_Code & "',N'" & Ratio_Share_Vishare & "',N'" & Ratio_Share_Clip & "',N'" & Ratio_Share_Anh & "',N'" & Status & "',N'" & Url & "',N'" & Create_Time & "',N'" & Create_By_Id & "',N'" & Create_By_Text & "',N'" & Update_Time & "',N'" & Update_By_Id & "',N'" & Update_By_Text & "',N'" & Description & "')"
            ElseIf ViewState(ViewStateInfo.Status) = Constants.Action.Update Then
                sql = "Update Vishare_DictIndex_Url_Service Set Partner_Id=N'" & Partner_Id & "'," & _
                     "Url_Code=N'" & Url_Code & "'," & _
                     "Ratio_Share_Vishare=N'" & Ratio_Share_Vishare & "'," & _
                     "Ratio_Share_Clip=N'" & Ratio_Share_Clip & "'," & _
                     "Ratio_Share_Anh=N'" & Ratio_Share_Anh & "'," & _
                     "Status=N'" & Status & "'," & _
                     "Url=N'" & Url & "'," & _
    ...
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDelete.Click
        Dim sql As String = "Delete From Vishare_DictIndex_Url_Service Where Id=" & ViewState(ViewStateInfo.Object_Id)
        Try
            MSSQLEnv.ExecuteNonQuery.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
            sql = "Delete From Ccare_Management_Contract_Vishare Where Url_Id=" & ViewState(ViewStateInfo.Object_Id)
            MSSQLEnv.ExecuteNonQuery.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
            RedirectUrl(Constants.Url._Vishare.VishareDictIndexUrlList)
        Catch ex As Exception
