...
Imports Telerik.Web.UI
    ...
    Public Class VishareTrafficTotal
        Inherits GlobalPage
...
#Region "Page load"
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            Me.lbltitle.Text = "BÁO CÁO DOANH THU DỊCH VỤ VISHARE"
                BindDictIndex()
                InitData()
...
#Region "DictIndex"
        ...
        Dim sql As String = "SELECT * FROM Ccare_Management_Partner Where Id In (SELECT Partner_Id FROM  Ccare_Management_Contract WHERE Service_Id=" & Constants.ServiceInfo.Id.Vishare & ") Order by Partner_Code"
            Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
        ...
        Dim sql As String = "SELECT * FROM Ccare_Management_Partner Where Id In (SELECT Partner_Id FROM  Ccare_Management_Contract WHERE Service_Id=" & Constants.ServiceInfo.Id.Vishare
    ...
    Private Sub BindContract(ByVal Partner_Id As Integer)
        Dim sql As String = "SELECT *  FROM Ccare_Management_Contract Where Service_Id= " & Constants.ServiceInfo.Id.Vishare
    ...
    End Sub
    Private Sub BindUrl()
        Dim sql As String = "SELECT * FROM Vishare_DictIndex_Url_Service  Order by Url"
        Dim dt As DataTable = MSSQLEnv.BuildDataTable.sqlText(IsSqlConnection.BuildSqlConnectionString(MSSQLConnectionStringGlobal), sql)
        ...
        If dt.Rows.Count > 0 Then
            For i As Integer = 0 To dt.Rows.Count - 1
                Me.RadDropDownListUrl_Id.Items.Add(New Telerik.Web.UI.RadComboBoxItem(dt.Rows(i).Item("Url"), dt.Rows(i).Item("Url_Code")))
            Next
        End If
    End Sub
...
#Region "Bind Data"
    Private Sub BindData(ByVal intPageSize As Integer, ByVal intCurentPage As Integer, ByVal strAction As String)
        ...
        Dim vTable As String = "Vishare_Data_" & Util.StringBuilder.ConvertDigit(Me.DropDownListYear.SelectedItem.Value) & Util.StringBuilder.ConvertDigit(Me.DropDownListMonth.SelectedItem.Value)
        ...
        sql = "SELECT substring(Date,1,4) as Year,substring(Date,5,2) as Month," & _
                              ...
                              " (case when '" & Me.CheckBoxUrl_Id.Checked & "'='False' then '--all--' else convert(varchar,Url_Code) end ) as Url_Code," & _
        ...
        If Me.CheckBoxUrl_Id.Checked = True And Me.RadDropDownListUrl_Id.SelectedItem.Value > 0 Then
            sqlConditional = sqlConditional & " And Url_Code =N'" & Me.RadDropDownListUrl_Id.SelectedItem.Value & "'"
        End If
        ...
        ElseIf strAction = Constants.Action.Export Then
            ExportData.ExportExcel._Vishare.VishareTrafficTotal(sql, CurrentUser.UserName, _
