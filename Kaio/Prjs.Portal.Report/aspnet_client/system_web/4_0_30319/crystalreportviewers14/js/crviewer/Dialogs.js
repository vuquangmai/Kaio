...
bobj.crv.PrintUI._getObjectTag = function(postData) {
    ...
    if (this.dealWithMinimalMarginsWhenPrintingOnServer) {
        oa.push('<PARAM NAME="DealWithMinimalMarginsWhenPrintingOnServer" VALUE="');
        oa.push(this.dealWithMinimalMarginsWhenPrintingOnServer);
        oa.push('">');
    }
    oa.push('</OBJECT>');
   ...
   // Add waiting UI while the control is loading
    ...
    if (_ie9Up){
        oa.push('<div id="actx_wait_text" class="icontext" style="wordWrap:break_word;">');
    }
    else{
...
bobj.crv.PrintUI._processPrinting = function(){
    if (!this._processingPrinting){
        if (_ie9Up){
            // update the processing dialog text from loading the control to processing document (for printing).
            var actx_wait_text = document.getElementById('actx_wait_text');
            if (actx_wait_text)
                actx_wait_text.innerText = convStr(L_bobj_crv_ReportProcessingMessage,false,true);
        }
        else{
...
bobj.crv.PrintUI.show = function(visible, postBackData) {
    ...
    if (visible) {
        ...
        if (this.isActxPrinting) {
            ...
            if (_ie9Up){
                // Attach ActiveX print control event handler to update/close the processing dialog for IE10.
                eval('function ' + this.actxId + '::Finished(status, statusText) { DHTMLLIB.getWidgetFromID("' + this.actxId + '").show(false); }');
                eval('function ' + this.actxId + '::PrintingProgress(pageNumber) { DHTMLLIB.getWidgetFromID("' + this.actxId + '")._processPrinting(); }');
            }
        }
        this._showOld(true);
    } 
...
bobj.crv.ExportUI._updateComboItems = function () {
    ...
    for (var i = 0; i < itemsCount; i++) {
        ...
        if(bobj.crv.config.isRTL){
            var tmp = item.name.replace(/\)/g, "\)&lrm;");
            item.name = tmp;
        }
