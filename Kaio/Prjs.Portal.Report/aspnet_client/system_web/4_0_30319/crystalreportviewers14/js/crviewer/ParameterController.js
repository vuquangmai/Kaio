...
bobj.crv.params.ParameterController.prototype = {
    ...
    /*
     * Private. Convert value text which could contains description to value structure based on default values of param
     * @param param   [bobj.crv.params.Parameter]    Value's data type (bobj.crv.params.DataTypes)
     * @param desc    [string] 
     */
    _getParameterValueFromDefaultValues: function(param, promptText) {
        if(param.defaultValues && bobj.isArray(param.defaultValues)) {
            for(var i = 0 ; i < param.defaultValues.length;i++) {
                // [ADAPT01712229] return the complete value structure instead of the simple value text representation
                var defPromptText = this._getDefaultValue(param.valueDataType, param.defaultDisplayType, param.defaultValues[i]);
                if(defPromptText == promptText) {
                    // The value text from the prompt matches that for a particular default value, so return the value structure
                    return param.defaultValues[i];
                }
            }
        }
        return null;
    ...
    _checkAndSetValue: function(widget, valueIdx) {
        ...
        // [ADAPT01712229] Previously, paramValue would be the unformatted string representation of the value.
        //         However, that would cause a loss of the description when applying the new value.
        //         Thus, we try to set it to the full parameter value structure before falling back on
        //         the simple value representation.  The methods that use paramValue below, validateValue()
        //         and _getValueTextFromDefaultValue(), appear to work with both value representations.
        var paramValue = this._getParameterValueFromDefaultValues(parameter, valueText);
        if (paramValue == null) {
            paramValue = this._getParamValue(parameter.valueDataType, valueText);
        }
        ...
        if (Status.OK === code) {
            ...
            if(cText != null) {
                ...
                // [ADAPT01712229] If we replace the text that the user entered into the widget, then re-get
                //         the parameter value structure so that we preserve the prompt value description.
                if (cText != valueText) {
                    var defaultValue = this._getParameterValueFromDefaultValues(parameter, cText);
